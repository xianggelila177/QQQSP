...
import {checkVersion,syncReadmeVersion} from './version.mjs';import {buildStatic} from './build-static.mjs';import {syncExample} from './env-example.mjs';
...
let count=0,shellLf=0,shellSkipped=0;for(const f of walk(root)){if(!/\.(?:mjs|js|sh)$/.test(f))continue;const shell=f.endsWith('.sh');if(shell){if(fs.readFileSync(f).includes(13))throw new Error('Shell source must use LF: '+f);shellLf++;if(process.platform==='win32'){shellSkipped++;continue;}}const r=spawnSync(shell?'bash':process.execPath,shell?['-n',f]:['--check',f],{encoding:'utf8'});if(r.status!==0||r.error)throw new Error(f+'\n'+(r.stderr||r.error));count++;}
...
buildContextDocs(root,{check:true});buildDetailDocs(root,{check:true});checkVersion(root);syncReadmeVersion(root,{check:true});buildStatic(root,{check:true});syncExample(root,{check:true});
...
console.log('Shell LF检查：'+shellLf+'；当前平台未执行bash语法检查：'+shellSkipped+'。');
