...
import {directoryEquities,equityAliases} from './equity-directory.js';
...
const instrumentIndex=new Map(directoryEquities.map(row=>[row.symbol,row]));
for(const row of instruments){
 const listed=instrumentIndex.get(row.symbol);
 instrumentIndex.set(row.symbol,{...listed,...row,aliases:[...new Set([...(listed?.aliases||[]),...(row.aliases||[]),...equityAliases(row.symbol)])]});
}
export const catalogInstruments=()=>[...instrumentIndex.values(),...futureInstruments];
export function catalogInstrumentFor(symbol){return futureInstrumentFor(symbol)||instrumentIndex.get(String(symbol||'').toUpperCase()) || null;}
