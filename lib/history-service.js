...
export function createHistoryService({fetchChart,now=()=>Date.now(),cacheTtl=60000,fullTtl=86400000,maxDaily=20000,maxEntries=24,maxBytes=48*1024*1024,maxConcurrent=1,maxQueued=16,deadlineMs=18000,totalDeadlineMs=20000,maxYears=45,failureCooldown=60000,maxFailureEntries=128}={}){
 ...
 async function ensureRaw(symbol,start){
  ...
  record.promise=enqueue(async()=>{
   ...
   if(owner===generation&&!closed&&!['HISTORY_CANCELLED','HISTORY_BUSY','REQUEST_CANCELLED'].includes(error.code)&&cache.has(symbol)){const old=cache.get(symbol);old.failedAt=now();old.error=error.code||'HISTORY_SOURCE_UNAVAILABLE';old.retryAt=Math.max(now()+60000,Number(error.retryAt)||0);}
   throw error;
 ...
 function restore(state){
  ...
  for(const item of state.entries.slice(0,maxEntries)){
   ...
   if(!date(meta.from)||!date(meta.through)||meta.from>meta.through||meta.listingDate!=null&&!date(meta.listingDate)||!timestamp(meta.checkedAt)||!timestamp(meta.fullCheckedAt)){skip('dates');continue;}
   if(!Array.isArray(meta.warnings)||meta.warnings.length>64||meta.warnings.some(v=>typeof v!=='string'||v.length>256)||!events(meta.sourceEvents)){skip('metadata');continue;}
   if(['originReached','budgetLimited','retrievalLimited'].some(k=>typeof meta[k]!=='boolean')||meta.failedAt!=null&&!timestamp(meta.failedAt)||meta.retryAt!=null&&(!Number.isFinite(meta.retryAt)||meta.retryAt<0)||meta.error!=null&&typeof meta.error!=='string'){skip('metadata');continue;}
   ...
   const rows=item.rows.map(([sessionDate,o,h,l,c,sourceVolume])=>normalizeDailyBar({sessionDate,o,h,l,c,v:null,sourceVolume,source:id.source,currency:id.currency,adjustmentBasis:id.adjustmentBasis,volumeUnit:id.volumeUnit,quality:'source-unverified'}));
   if(rows.some((b,i)=>!b||b.sessionDate<meta.from||b.sessionDate>meta.through||i&&rows[i-1].sessionDate>=b.sessionDate)){skip('rows');continue;}
   const restored=Object.fromEntries(['identity','seriesId','from','through','listingDate','originReached','checkedAt','fullCheckedAt','sourceEvents','warnings','budgetLimited','retrievalLimited','failedAt','error','retryAt'].filter(k=>Object.hasOwn(meta,k)).map(k=>[k,meta[k]]));
   remember(symbol,{...restored,rows});
  }
 }
 ...
 return {get,prepare,exportState,restore,close,reopen,clear,cache,diagnostics:()=>({entries:cache.size,failureEntries:failures.size,bytes,byteAccounting:'raw+four-aggregate-reservation',maxBytes,aggregateBuilds,inflight:rawInflight.size,...taskQueue.diagnostics(),restoreSkipped,closed})};
}
