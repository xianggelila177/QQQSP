...
export function createQuoteCache({cacheMap=new Map(),inflight=new Map(),failAt=new Map(),fetchQuote,usFallbackQuote,
  ...
  const resource=createCachedResource({cache:cacheMap,inflight,failures:failAt,ttlMs:cacheMs,retryMs:60000,maxEntries:MAX_WATCHLIST_SYMBOLS,now,loader:async symbol=>{
