...
// Some cash snapshot feeds publish the completed regular session after its
// closing bell. Their date still scopes the source comparison, but this must
// never turn a pre-open maintenance timestamp into a regular-session anchor.
export function regularSnapshotReferenceAt(symbol,quoteAt,{allowPublishedClose=false}={}){
  const at=stamp(quoteAt),date=tradeDate(symbol,at);
  if(!at||!date)return null;
  let day;
  try{day=tradingDayInfo(symbol,date);}catch{return null;}
  if(!day.known||!day.is_open||!day.regular_sessions.length)return null;
  if(day.regular_sessions.some(s=>at>=s.open_at_ms&&at<s.close_at_ms+60000))return at;
  return allowPublishedClose&&at>=day.regular_sessions.at(-1).close_at_ms?at:null;
}
...
export function resolvePreviousCloseReference(quote){
  ...
  if(providedDate){
    ...
    if(positive(quote.prevClose)){
      ...
      // A prior publication already derives a date for source-aligned values.
      // That internal date is not new source evidence on the next publication.
      status=['regular-close','source-aligned','source-dated'].includes(quote.previousCloseStatus)?quote.previousCloseStatus:'source-dated';
    }
  }
  ...
  if(value===null)return missing(quote.previousCloseStatus==='unavailable'&&quote.previousCloseMissingReason?quote.previousCloseMissingReason:
    providedDate?'PREVIOUS_CLOSE_UNAVAILABLE':quote.priceSession==='PRE'?'PRIOR_REGULAR_CLOSE_UNCONFIRMED':'PREVIOUS_CLOSE_DATE_UNCONFIRMED',expectedDate);
