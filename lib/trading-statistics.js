import {financialNumber} from './fundamentals.js';
...
export function regularTradingStatistics(quote){
  ...
  const raw=explicit?.session==='REGULAR'?explicit:['REGULAR','CACHED_REGULAR','CN_SNAPSHOT'].includes(quote.ohlcSession)?{session:'REGULAR',asOf:quote.regularQuoteAt??quote.quoteAt??quote.ts,open:quote.open,high:quote.dayHigh,low:quote.dayLow,prevClose:quote.prevClose,volume:quote.volume,volumeUnit:quote.volumeUnit,turnoverAmount:quote.turnoverAmount,currency:quote.currency,source:quote.statisticsSource||quote.src}:null;
