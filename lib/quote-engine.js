...
export function createQuoteEngine({readQuote,now=Date.now,onMembership=()=>{},tickMs=1000,leaseMs=15000}={}) {
  ...
  function commit(symbol,value) {
    ...
    if(previous?.price!=null&&at(previous)>at(value)&&!String(value.realtimeStatus||'').includes('INVALIDATED')) {
      ...
      value={...previous,realtimeStatus:value.realtimeStatus??previous.realtimeStatus,...(value.stale?{stale:true,staleInfo:value.staleInfo}:{})};
    }
