...
export function createQuoteEngine({readQuote,decorateQuote=value=>value,now=Date.now,onMembership=()=>{},tickMs=1000,leaseMs=15000}={}) {
  const quotes=new Map(),signatures=new Map(),inflight=new Map(),clients=new Map(),leases=new Map(),listeners=new Set();
  let timer=null,flushTimer=null,running=false,epoch=0;const dirty=new Set();
  ...
  function commit(symbol,value) {
    ...
    if(previous?.price!=null&&at(previous)>at(value)&&!String(value.realtimeStatus||'').includes('INVALIDATED')) {
      ...
      value={...mergeQuoteHistory(previous,value),realtimeStatus:value.realtimeStatus??previous.realtimeStatus,...(value.stale?{stale:true,staleInfo:value.staleInfo}:{})};
    }
  ...
  function update(symbol) {
    if(!running||inflight.has(symbol))return inflight.get(symbol);
    const owner=epoch;
    const job=Promise.resolve().then(()=>readQuote(symbol)).then(value=>{
      ...
      .finally(()=>{if(inflight.get(symbol)===job)inflight.delete(symbol);});
  ...
  function poke(symbol) {if(active().has(symbol))void update(symbol);}
  return {read,watch,poke,tick,subscribe(listener){listeners.add(listener);return ()=>listeners.delete(listener);},
    ...
    stop(){running=false;epoch++;clearInterval(timer);clearTimeout(flushTimer);timer=flushTimer=null;clients.clear();leases.clear();dirty.clear();inflight.clear();quotes.clear();signatures.clear();onMembership([]);},
