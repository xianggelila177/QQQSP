  ...
  const job=(async()=>{
   ...
   try{
    ...
    if(/^\d{6}\.(SS|SZ)$/.test(symbol)){
     ...
     value={bars,source:chart.source||'yahoo',currency:chart.meta?.currency,...(chart.meta?.chartTimeContract?{timeContract:chart.meta.chartTimeContract,timeBasis:chart.meta.chartTimeBasis,timeZone:chart.meta.chartTimeZone}:{})};
    }
