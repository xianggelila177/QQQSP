...
export function createQuoteService({ log = defaultLog, now = () => Date.now(), barsFrom = parseChartBars, fxFallbackRates = () => ({}), fxMetadata = () => ({fxAsOf:null,fxStale:true}), txDailyBarsCn, txMinuteBarsCn, txQuoteSnapshot, usSnapshot, emQuoteSnapshot, getNasdaqDaily, cacheSet = boundedCacheSet, extSessions, providers = {} } = {}) {
  ...
  async function fallbackQuote(symbol) {
    ...
    const base = snap.prevClose != null ? snap.prevClose : null;
    const change = snap.change != null ? snap.change : (base != null ? snap.price - base : null);
    const changePct = snap.changePct != null ? snap.changePct : (base ? (change / base) * 100 : null);
    ...
    return publishQuote({
      ...
      ts: snap.ts, price: snap.price, prevClose: base, open: snap.open, dayHigh: snap.dayHigh, dayLow: snap.dayLow,
  ...
  async function fetchQuote(symbol) {
    ...
    return publishQuote({
      ...
      price, prevClose: prev, open: finiteOrNull(openOut), dayHigh: finiteOrNull(highOut), dayLow: finiteOrNull(lowOut),
