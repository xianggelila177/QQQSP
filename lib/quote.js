...
import {previousCloseFromDailyBars,regularSnapshotReferenceAt} from './previous-close-reference.js';
...
export function createQuoteService({ log = defaultLog, now = () => Date.now(), barsFrom = parseChartBars, fxFallbackRates = () => ({}), fxMetadata = () => ({fxAsOf:null,fxStale:true}), txDailyBarsCn, txMinuteBarsCn, txQuoteSnapshot, usSnapshot, emQuoteSnapshot, getNasdaqDaily, cacheSet = boundedCacheSet, extSessions, providers = {} } = {}) {
  ...
  async function fallbackQuote(symbol) {
    ...
    if (/^\d{6}\.(SS|SZ)$/.test(symU)) {
      ...
      const regularQuoteAt=regularSnapshotReferenceAt(symU,cn.ts,{allowPublishedClose:true});
      return publishQuote({
        ...
        marketState: marketStateFor(symU,28800,now()), calendarCoverage: marketCalendarCoverage(symU,now()), change: changeC, changePct: baseC ? (changeC / baseC) * 100 : null, instrumentType: cnType,instrumentTypeSource:cnMeta.instrumentTypeSource,priceSession:'REGULAR',regularPrice:regularQuoteAt?cn.price:null,regularQuoteAt,ext: null,
        ...(regularQuoteAt&&priceSessionFor(symU,cn.ts)!=='REGULAR'?{quoteTimeBasis:'provider-published'}:{}),
    ...
    const priceSession=snap.priceSession ?? priceSessionFor(symU,snap.ts);
    const regularQuoteAt=priceSessionFor(symU,snap.ts)==='REGULAR'?snap.ts:null;
    const base = dailyReference?.prevClose??(regularQuoteAt?finiteOrNull(snap.prevClose):null);
    ...
    return publishQuote({
      ...
      marketState: marketStateFor(symU,null,now()), calendarCoverage: marketCalendarCoverage(symU,now()), change, changePct,instrumentType:usMeta.instrumentType,instrumentTypeSource:usMeta.instrumentTypeSource,priceSession,regularPrice:regularQuoteAt?snap.price:null,regularQuoteAt,ext:null,
  ...
  async function fetchQuote(symbol) {
    ...
    const regularQuoteAt=cnSnap?regularSnapshotReferenceAt(symbol,cnSnap.ts,{allowPublishedClose:true}):
      priceSessionFor(symbol,quoteAtMeta,null,sessionContext)==='REGULAR'?quoteAtMeta:null;
    ...
    return publishQuote({
      ...
      regularQuoteAt,...(cnSnap&&regularQuoteAt&&priceSessionFor(symbol,cnSnap.ts)!=='REGULAR'?{quoteTimeBasis:'provider-published'}:{}),
