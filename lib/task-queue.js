// A bounded FIFO. Cancellation removes waiting work immediately. An active slot
...
export function createTaskQueue({maxActive=1,maxQueued=32,now=Date.now}={}) {
  ...
  function run(fn,{signal,deadlineAt}={}) {
    ...
    const promise=new Promise((resolve,reject)=>{
      ...
      job={fn,controller,started:false,queuedAt:now(),finish(err,value){
  ...
  return {run,close(){closed=true;for(const job of [...waiting,...running])job.cancel(error('STOPPED','Queue stopped'));},reopen(){closed=false;drain();},
    diagnostics:()=>({active:running.size,queued:waiting.length,maxActive,maxQueue:maxQueued,rejected,cancelled,completed,waitTotalMs,waitMaxMs,oldestQueuedMs:waiting.length?Math.max(0,now()-waiting[0].queuedAt):0,closed})};
}
