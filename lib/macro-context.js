...
const validPrice=q=>q&&typeof q.price==='number'&&Number.isFinite(q.price)&&q.price>0;
const stamp=n=>typeof n==='number'&&Number.isFinite(n)&&n>0?n:null;
const quoteTime=q=>stamp(q?.quoteAt);
const fields=['symbol','price','quoteAt','sourceCheckedAt','src','source','currency','priceBasis','contractSymbol','quoteTradingDate','tradingDate','feedDelayMinutes','feedCoverage','stale','recovery','pollAfterMs','unit','displayName','observationDate','daily','proxy','providerSymbol','sourceTimeText','sourceTimeZone','sourceUrl','sourcePollAfterMs','selectedSource','diagnostics'];
export function createMacroContext({readQuote,now=Date.now,pollMs=30000}={}){
 ...
 const tasks=new Map(),listeners=new Set();let closed=false;
 ...
 function accept(f,s,value){
  ...
  if(q.stale||q.recovery||q.daily||q.feedDelayMinutes>0)return;
 ...
 function windowsFor(f,q,array,end,basis){return WINDOWS.map(ms=>{
  ...
  if(!basis||!anchor)return {windowMs:ms,change:null};
 ...
 function snapshot(){
  ...
  const factors=MACRO_FACTORS.map(f=>{
   ...
   const clean=validPrice(q)&&!(q.feedDelayMinutes>0)&&!q.stale&&!q.recovery&&!q.daily&&!s.error;
   ...
   const status=s.error?'error':!q?(loading?'loading':'unavailable'):q.daily?(dailyExpired?'stale':'daily'):!basis?'stale':selected?'ready':'warming';
   const unavailableCached=status==='stale'&&s.diagnostics.some(d=>['error','cooldown'].includes(d.status));
   const availability=unavailableCached?'来源暂不可用，保留旧价；按冷却时间重试':status==='error'?'请求失败，已保留可用缓存；按冷却时间重试':status==='daily'?'日度参考，不用于分钟比较':status==='stale'?'来源陈旧、延迟或时间未知':basis==='observation'?'服务器观察采样，非成交时间':basis==='source'?'按来源时间采样':loading?'正在请求候选来源':'尚未取得来源';
   return {...f,name:q?.displayName||f.name,unit:q?.unit||f.unit,symbol:q?.symbol||f.symbol,requestedSymbol:f.symbol,
    price:validPrice(q)?q.price:null,quoteAt:t,sourceCheckedAt:checked,source:q?.src||q?.source||null,sourceUrl:q?.sourceUrl||null,ageMs:t?Math.max(0,clock-t):null,
 ...
 function startDue(){
  ...
  for(const f of MACRO_FACTORS){const s=states.get(f.id);if(tasks.has(f.id)||now()<s.nextAt)continue;
   const started=now();s.attemptAt=started;s.nextAt=started+pollMs;
   const task=Promise.resolve().then(()=>readQuote(f.symbol)).then(q=>{
    if(closed)return;accept(f,s,q);s.nextAt=Math.max(s.nextAt,started+Number(q?.pollAfterMs||0));
   }).catch(e=>{if(!closed){s.error=String(e?.message||e).slice(0,240);s.diagnostics=e.diagnostics||[{source:f.symbol,status:'error',code:e.code||'NETWORK',message:s.error,retryAt:e.retryAt}];s.nextAt=Math.max(now()+60000,Number(e?.retryAt)||0);}}
   ).finally(()=>{if(tasks.get(f.id)===task)tasks.delete(f.id);notify();});
   tasks.set(f.id,task);
  }
 }
 async function getContext(){startDue();await Promise.allSettled([...tasks.values()]);return snapshot();}
 function restore(data){if(!data||data.version!==1||!Array.isArray(data.states))return;
  for(const [id,value]of data.states){const s=states.get(id);if(!s||!validPrice(value?.quote))continue;
   ...
   const valid=arr=>Array.isArray(arr)?arr.filter(p=>p&&stamp(p.t)&&p.t<=now()+1000&&p.t>=now()-RETENTION&&validPrice(p)).slice(-180):[];
   s.samples=valid(value.samples);s.observations=valid(value.observations);
  }
 }
 return {getContext,snapshot,exportState:()=>({version:1,states:[...states]}),restore,
  subscribe(fn){listeners.add(fn);return()=>listeners.delete(fn);},
  requestContext(){startDue();return snapshot();},close(){closed=true;},reopen(){closed=false;},
  clear(){for(const s of states.values())Object.assign(s,{quote:null,samples:[],observations:[],error:null,nextAt:0,identity:null,diagnostics:[]});}};
}
