...
export function createApiQuoteStream({engine,keys,budget=createStreamBudget(),now=Date.now,leaseMs=0,tickMs=1000}={}){
 ...
 function open(req,res,symbols,principal,headers){
  ...
  // Admit membership before sending SSE headers so callers receive a normal
  // retryable JSON error when the global quote budget is exhausted.
  try{release=engine.watch(symbols);}catch(error){budget.release(res);throw error;}
  ...
  try{
   ...
   if(!write('retry: 1000\n\n'))return;unlisten=engine.subscribe(changed=>{if(changed.some(s=>symbols.includes(s)))dirty=true;});
