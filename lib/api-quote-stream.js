...
// not an unbounded replay log. Each lease is at most 60 seconds.
export function createApiQuoteStream({engine,keys,budget=createStreamBudget(),now=Date.now,leaseMs=60000,tickMs=1000}={}){
 ...
 function open(req,res,symbols,principal,headers){
  ...
  let release,unlisten,timer,deadline,lastSent=0,dirty=true,closed=false;
  ...
  const tick=()=>{
   ...
   if(dirty){
    ...
    event('quote',{schema_version:1,generated_at_ms:at,results,delivery:'coalesced snapshots; intermediate ticks may be omitted'});lastSent=at;}catch{event('source_error',{code:'SOURCE_UNAVAILABLE'});close();return;}
   }else if(now()-lastSent>=15000){budget.write(res,': heartbeat\n\n');lastSent=now();}
  };
  try{
   ...
   budget.write(res,'retry: 5000\n\n');release=engine.watch(symbols);unlisten=engine.subscribe(changed=>{if(changed.some(s=>symbols.includes(s)))dirty=true;});
   for(const symbol of symbols)engine.poke(symbol);tick();
   if(!closed){timer=setInterval(tick,tickMs);timer.unref?.();deadline=setTimeout(()=>{event('end',{code:'LEASE_ENDED',reconnect_after_ms:5000});close();},leaseMs);deadline.unref?.();}
  }catch(e){close();throw e;}
 }
 return {open,close(){for(const close of [...connections])close();},diagnostics:()=>({connections:connections.size,maxConnections:16,maxPerKey:2,leaseMs,tickMs})};
}
