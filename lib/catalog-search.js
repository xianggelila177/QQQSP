...
export function catalogSearch(query){
 const q=key(canonicalSearchQuery(query));if(!q)return [];
 return index.map(item=>{
  ...
  return {symbol:row.symbol,name:row.name,type:row.type,market:market.label,exch:row.exchange||market.exchange,
   ...
   ...(row.identitySource?{identitySource:row.identitySource,listingAsOf:row.listingAsOf,listingSection:row.listingSection,quoteCoverage:row.quoteCoverage}:{}),
