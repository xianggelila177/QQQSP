...
export function aggregateHistory(rows,period,options={}){
 ...
 for(const b of [...byDate.values()].sort((a,z)=>a.sessionDate.localeCompare(z.sessionDate))){
  ...
  const g=groups.get(key)||{...bounds,t:dateMs(key)/1000,firstTradingDate:b.sessionDate,lastTradingDate:b.sessionDate,o:b.o,h:b.h,l:b.l,c:b.c,v:0,knownVolume:0,knownVolumeBars:0,volumeComplete:true,volumeUnit:b.volumeUnit,dates:new Set(),source:b.source,currency:b.currency};
  g.lastTradingDate=b.sessionDate;g.h=Math.max(g.h,b.h);g.l=Math.min(g.l,b.l);g.c=b.c;g.dates.add(b.sessionDate);
  if(b.v==null)g.volumeComplete=false;else {g.knownVolume+=b.v;g.knownVolumeBars++;}
  groups.set(key,g);
 }
 return [...groups.values()].map(g=>{
  ...
  const {dates,...bar}=g;
  return {...bar,v:g.volumeComplete?g.knownVolume:null,volumeStatus:g.volumeComplete?'ready':g.knownVolumeBars?'partial':'missing',
    volumeCoverage:{knownBars:g.knownVolumeBars,totalBars:g.dates.size,knownVolume:g.knownVolume,unit:g.volumeUnit},
    periodState:periodStatus(g,{symbol,zone,asOf,dayPlan}),coverageStatus:coverage,qualityFlags:flags};
 });
}
