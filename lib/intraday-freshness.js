...
function lastPoint(quote,now){
  return (quote.regularChart?.bars||[]).findLast(p=>Number.isFinite(p?.t)&&p.t>0&&p.t*1000<=now&&Number.isFinite(p.c)&&p.c>0);
}
...
export function intradayLiveFields(quote,{now=Date.now(),historyCurrency}={}){
  ...
  if(quote.stale||quote.staleInfo||quote.pending||quote.recovery||quote.error||
    ...
    !active.includes(quote.marketState)||quote.priceSession!=='REGULAR'||now-at>freshnessMs||
  ...
  if(!historyCurrency||quote.currency!==historyCurrency||quote.regularChart?.source!==quote.src)return empty('unavailable');
  ...
  if(!sessionDate||sessionDate!==historyDate||quote.regularChart?.tradeDate!==sessionDate||
    !quote.regularChart?.regularSessions?.some(s=>at>=s.open_at_ms&&at<=s.close_at_ms))return empty('waiting-history');
