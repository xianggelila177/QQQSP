...
export function createMacroMonitor({macro,context,calendar,now=Date.now,enabled=true,statePath='',contextMs=30000,newsMs=60000,tickMs=1000,notify=null,log={warn(){}}}={}){
 ...
 function snapshot(){return {schemaVersion:1,revision,serverNow:now(),news:{...news,refreshing:!!lanes.news.task},context:{...context.snapshot(),calendar:calendar.snapshot()},monitor:status()};}
 ...
 context.subscribe?.(()=>{if(running)emit();});
 ...
 async function restore(){if(!statePath)return;try{
  ...
  if(data.news&&Array.isArray(data.news.items)){news={...data.news,items:data.news.items.filter(x=>x?.title&&Number.isFinite(x.t)&&x.t<=now()&&now()-x.t<48*3600e3).slice(0,100),stale:true,restored:true};news.items=news.items.map(x=>({...x,assessment:assessMacro(x,now())}));}
 ...
 function recordNews(data){
  ...
  const merged=mergeMacroItems([...current,...news.items]).filter(x=>Number.isFinite(x.t)&&now()-x.t<48*3600e3&&x.t<=now()).slice(0,100).map(x=>({...x,assessment:assessMacro(x,now())}));
 ...
 function launch(name,fn,interval){const lane=lanes[name];if(!running||lane.task||now()<lane.nextAt)return;
  ...
  lane.task=Promise.resolve().then(fn).then(value=>{if(!running)return;if(name==='news')recordNews(value);lane.completedAt=now();
 ...
 function runDue(){if(!running)return;launch('context',()=>context.getContext(),contextMs);launch('news',()=>macro.getMacro({waitForRefresh:true}),newsMs);launch('calendar',()=>calendar.getCalendar(),60000);deliver();}
 ...
 async function stop(){running=false;clearTimeout(timer);timer=null;await init;await Promise.allSettled([...Object.values(lanes).map(l=>l.task),notificationTask].filter(Boolean));await save();await saveChain;}
 return {start,stop,runDue,snapshot,status,subscribe(fn){listeners.add(fn);return()=>listeners.delete(fn);},
  async settled(){await init;await Promise.allSettled(Object.values(lanes).map(l=>l.task).filter(Boolean));},
