...
export function selectFinancialRecord(symbol,entries,sources,{now,maxAgeMs}={}){
  ...
  const pairs=(candidates.get('week52High')||[]).flatMap(high=>{const low=(candidates.get('week52Low')||[]).find(v=>v.field.providerId===high.field.providerId&&v.field.value<=high.field.value);return low?[{...high,low:low.field}]:[];}).sort(sort);
...
export function deriveFinancialFields(quote,record){
  ...
  const fields={...record.fields},base=quote.currency==='GBp'||quote.currency==='GBX'?['GBP',.01]:quote.currency==='ZAc'||quote.currency==='ZAC'?['ZAR',.01]:[quote.currency,1];
