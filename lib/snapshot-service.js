...
export function createSnapshotService({ fetchBatch, enrich, now = Date.now, env = {}, sessionFor, streamAvailable = () => false, onUpdate=()=>{} } = {}) {
  ...
  async function refreshGroup(group, symbols, epoch) {
    ...
    try {
      ...
      for (const symbol of symbols) {
        ...
        snapshots.set(symbol, publishQuote({ ...quote }));
      ...
      for (const symbol of symbols) if (active.has(symbol)) {
        ...
        state.dueAt.set(symbol,startedAt+Math.max(backoff,delayOf(hint),['CLOSED','HOLIDAY'].includes(session(symbol))?60000:session(symbol)==='BREAK'?30000:0));
      }
  ...
  function getCachedQuote(value) {
    ...
    if (!quote) return { symbol, error: '等待首次报价', code: 'PENDING', pending: true };
