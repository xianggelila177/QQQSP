import { providerCapabilities, quoteDisplayName, validWeek52Range } from './instruments.js';
...
const GROUPS = ['us', 'kr', 'jp', 'other', 'index'];
...
export function createSnapshotService({ fetchBatch, enrich, now = Date.now, env = {}, sessionFor, streamAvailable = () => false, onUpdate=()=>{} } = {}) {
  ...
  function coldFailure(symbol,error,retryAt) {
    ...
    initialFailures.set(symbol,{symbol,pending:false,code:limited?'RATE_LIMITED':'SOURCE_UNAVAILABLE',
      error:limited?'行情来源限流，等待重试':'行情来源暂不可用，等待重试',failedAt:now(),retryAt:Math.max(now()+pollMs,Number(retryAt)||now()+60000)});
    onUpdate(symbol);
  }
  async function refreshGroup(group, symbols, epoch) {
    ...
    try {
      ...
      for (const symbol of symbols) {
        ...
        const old = snapshots.get(symbol);
        if (quoteTime(old) && (quoteTime(quote) < quoteTime(old)||quoteTime(quote)===quoteTime(old)&&Number(quote.sourceCheckedAt||0)<Number(old?.sourceCheckedAt||0))) continue;
        snapshots.set(symbol, publishQuote(mergeTradingStatistics(quote,old,{now:now()})));
        initialFailures.delete(symbol);
        onUpdate(symbol);successes++;
      }
      ...
      for (const symbol of symbols) if (active.has(symbol)) {
        ...
        if(!usable(quote))coldFailure(symbol,quote,state.dueAt.get(symbol));
      }
  ...
  function getCachedQuote(value) {
    ...
    if (!quote) return initialFailures.get(symbol) || { symbol, error: '等待首次报价', code: 'PENDING', pending: true };
  ...
  async function forceRefresh(symbols) {
    ...
    for(const symbol of requested)getCachedQuote(symbol);
    const epoch=generation,outcomes={};
    const batches=GROUPS.map(group=>[group,requested.filter(symbol=>groupOf(symbol)===group)]).filter(([,list])=>list.length);
    ...
    await Promise.all(batches.map(async ([group,list])=>{
      try {
        ...
        for(const symbol of list){
          const quote=rows.get(symbol),old=snapshots.get(symbol);
          if(!usable(quote)||quote.stale||quote.staleInfo||quote.recovery){outcomes[symbol]='unavailable';continue;}
          const currentAt=quoteTime(old),nextAt=quoteTime(quote);
          if(nextAt<currentAt||nextAt===currentAt&&Number(quote.sourceCheckedAt||0)<Number(old?.sourceCheckedAt||0)){
            outcomes[symbol]='older';continue;
          }
          const snapshot=publishQuote(mergeTradingStatistics(quote,old,{now:now()}));
          snapshots.set(symbol,snapshot);priceFallback.set(symbol,{quote:snapshot,checkedAt:now()});initialFailures.delete(symbol);
          const due=Number(result?.nextPollAtBySymbol?.[symbol]);
          if(Number.isFinite(due)&&due>now())groups.get(group)?.dueAt.set(symbol,due);
          outcomes[symbol]=nextAt>currentAt?'updated':'unchanged';onUpdate(symbol);
        }
