...
export function buildMarketContext({query,requestId,generatedAt,quote,daily,samples,news,macro,intraday,corporate_actions,errors={}}){
 ...
 function quoteSection(){
  ...
  const ext=(value,price,time)=>({price:positive(value?.price)??positive(price),quote_at_ms:timestamp(value?.quoteAt)??seconds(value?.t)??legacyTime(value?.time)??timestamp(time),change:number(value?.change),change_percent:number(value?.changePct),reference_price:positive(value?.referencePrice),reference_trade_date:date(value?.referenceTradeDate)});
