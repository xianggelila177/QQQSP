...
export function buildMarketContext({query,requestId,generatedAt,quote,daily,samples,news,macro,intraday,corporate_actions,errors={}}){
 ...
 function quoteSection(){
  ...
  return {...common({status,sources:usable?[quoteSource()]:[],asOf:validEventAt,checkedAt:q?.sourceCheckedAt,delay:q?.feedDelayMinutes,coverage:{kind:'latest_price_snapshot',sequence_scope:'not_provided',exchange_sequence:null,session:text(q?.priceSession),trading_date:date(q?.quoteTradingDate||q?.tradingDate)||localDate(validEventAt,zone)},adjustment:'not_applicable',reason:!usable?sectionError('quote',q?.pending?'QUOTE_PENDING':'NO_QUOTE'):eventReason||checkReason||(stale(q)?'RETAINED_QUOTE':null)}),data:usable?{
   price:q.price,currency:nativeCurrency,price_unit:unit,previous_close:number(q.prevClose),change:number(q.change),change_percent:number(q.changePct),quote_at_ms:timestamp(q.quoteAt),observed_at_ms:timestamp(q.observedAt),source_checked_at_ms:timestamp(q.sourceCheckedAt),quote_time_basis:text(q.quoteTimeBasis)||'source_time',session:text(q.priceSession),market_state:currentState,
