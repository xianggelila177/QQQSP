...
export function buildMarketContext({query,requestId,generatedAt,quote,daily,samples,news,macro,intraday,corporate_actions,errors={}}){
 ...
 const q=derivePaidDividends(quote&&(!quote.symbol||quote.symbol===symbol)?quote:null,corporate_actions,generatedAt);
 ...
 const nativeCurrency=text(q?.currency)||text(intraday?.currency)||text(d?.sourceCurrency)||(d?.currency!=='POINTS'?text(d?.currency):null)||sampleCurrency;
 ...
 function quoteSection(){
  ...
  const streamHealthy=quoteConnectionHealthy(q,generatedAt);
  const checkReason=q?.sourceCheckedAt==null?'UNKNOWN_SOURCE_CHECK':checked===null||checked>generatedAt?'SOURCE_TIME_INVALID':!streamHealthy&&generatedAt-checked>checkLimit?'SOURCE_CHECK_OVERDUE':null;
  const eventAt=timestamp(q?.quoteAt),ageLimit=quoteAgeLimitMs(q,generatedAt);
  const eventReason=q?.quoteAt==null?'UNKNOWN_QUOTE_TIME':eventAt===null||eventAt>generatedAt?'QUOTE_TIME_INVALID':generatedAt-eventAt>ageLimit?'QUOTE_TOO_OLD':null;
  const validEventAt=eventReason==='UNKNOWN_QUOTE_TIME'||eventReason==='QUOTE_TIME_INVALID'?null:eventAt;
  const status=!usable?'unavailable':eventReason||checkReason||stale(q)||q?.error||q?.pending||errors.quote?'partial':'ready';
  ...
  return {...common({status,sources:usable?[quoteSource()]:[],asOf:validEventAt,checkedAt:q?.sourceCheckedAt,delay:q?.feedDelayMinutes,coverage:{kind:'latest_price_snapshot',sequence_scope:'not_provided',exchange_sequence:null,session:text(q?.priceSession),trading_date:date(q?.quoteTradingDate||q?.tradingDate)||localDate(validEventAt,zone)},adjustment:'not_applicable',reason:!usable?sectionError('quote',q?.pending?'QUOTE_PENDING':'NO_QUOTE'):eventReason||checkReason||(stale(q)?'RETAINED_QUOTE':null)}),data:usable?{
 ...
 function dailySection(){
  ...
  const mapped=list(d?.bars).flatMap(bar=>{
   ...
   if(!at||!tradeDate||tradeDate>today||![bar.o,bar.h,bar.l,bar.c].every(value=>positive(value)!==null)||bar.l>Math.min(bar.o,bar.c)||bar.h<Math.max(bar.o,bar.c)){rejected++;return [];}
