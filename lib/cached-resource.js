...
export function createCachedResource({loader,ttlMs=60000,retryMs=60000,maxEntries=64,now=Date.now,cache=new Map(),inflight=new Map(),failures=new Map(),staleOnError=true}={}) {
  ...
  function refresh(key,options={}) {
    ...
    const owner=epoch;
    const job=Promise.resolve().then(()=>loader(key,{signal})).then(data=>{
      ...
      if(!closed&&owner===epoch&&!signal.aborted) {
  ...
  async function get(key,{swr=false,maxAgeMs=ttlMs,signal}={}) {
    if(closed)throw stopped();signal?.throwIfAborted();const entry=cache.get(key);
    if(entry&&now()-entry.ts<maxAgeMs)return entry.data;
    const job=refresh(key,{signal});
    if(entry&&swr){void job.catch(()=>{});return entry.data;}
    try{return await job;}catch(error){if(entry&&staleOnError&&!signal?.aborted&&error.code!=='STOPPED')return entry.data;throw error;}
  }
  return {get,refresh,cache,inflight,failures,close(){closed=true;epoch++;shared.close();inflight.clear();},reopen(){closed=false;},clear(){cache.clear();failures.clear();}};
}
