...
export function parseCv(raw) {
  ...
  for(const segment of segments) {
    const m=/^([A-Z0-9.\-^=&]{1,16}):(\d+):(\d+)(?::(\d+))?$/.exec(segment);
    if(!m || m.slice(2).some(v=>v!==undefined&&!Number.isSafeInteger(Number(v)))) return new Map();
    map.set(m[1],{iVer:Number(m[2]),dVer:Number(m[3]),...(m[4]===undefined?{}:{rVer:Number(m[4])})});
  }
  return map;
}
export function applyChartVersions(list,cvMap) {
  return list.map(q=>{
    ...
    if(q.regularChart){
      const revision=chartRevision(q.regularChart.bars);
      out.regularChart={...q.regularChart,revision};
      if(revision>0&&want?.rVer===revision)out.regularChart.bars='same';
    }
    if(q.charts) {
      out.charts={...q.charts};
