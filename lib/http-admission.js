...
export function createAdmission({env = {}, now = Date.now} = {}) {
  ...
  const drain = () => {
    while (active < maxActive && queue.length) {
      ...
      item.started = true; active++;
      Promise.resolve().then(item.fn).then(item.resolve, item.reject).finally(() => {active--; drain();});
    }
  };
  const run = fn => {
    if (active >= maxActive && queue.length >= maxQueue) {
      rejected++;
      return Promise.reject(Object.assign(new Error('request capacity exhausted'), {code:'CAPACITY_EXCEEDED',statusCode:503}));
    }
    let item;
    const promise = new Promise((resolve, reject) => {
      item = {fn,resolve,reject,started:false,queuedAt:now()}; queue.push(item); drain();
    });
    promise.cancel = () => {
      ...
      item.reject(Object.assign(new Error('request cancelled'), {code:'REQUEST_CANCELLED'}));
    };
    return promise;
  };
  ...
  const diagnostics = () => ({active,queued:queue.length,rejected,maxActive,maxQueue,oldestQueuedMs:queue.length?Math.max(0,now()-queue[0].queuedAt):0,clients:clients.size,snapshotLimit,expensiveLimit});
  return Object.freeze({run,consume,diagnostics});
}
