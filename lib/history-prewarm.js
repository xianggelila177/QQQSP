...
export function createHistoryPrewarm({history,now=Date.now,enabled=true,statePath='',refreshMs=60000,tickMs=1000,maxSymbols=24,expandAfterMs=5000,canExpand=()=>true,log={warn(){}}}={}){
 ...
 const clean=symbols=>[...new Set(symbols.filter(validSymbol))].slice(0,12);
 function reconcile(){
 ...
 function commit(symbol,periods,tier){
  ...
  for(const p of Object.keys(periods))if(old.periods?.[p]?.revision===periods[p].revision)periods[p]={...periods[p],bars:old.periods[p].bars};
  entries.set(symbol,{...old,symbol,periods,tier:tier||old.tier,expandAt:(tier||old.tier)==='long'?null:old.expandAt||now()+expandAfterMs,revision:changed,status:stale?'stale':'ready',checkedAt:now(),nextAt:Math.max(now()+refreshMs,Number(error?.retryAt)||0),retryAt:error?.retryAt||null,error:error?.errorCode||null});
  emit(symbol);saveSoon();
 }
 function runDue(){
  if(!enabled||!running||task)return task;
  const values=[...entries.values()];
  let entry=values.filter(e=>e.nextAt<=now()).sort((a,b)=>Number(!!a.periods)-Number(!!b.periods)||a.nextAt-b.nextAt)[0],tier='near';
  if(!entry&&canExpand()&&values.every(e=>e.periods||e.retryAt>now())){entry=values.find(e=>e.periods&&e.tier!=='long'&&e.expandAt<=now()&&!(e.retryAt>now()));tier='long';}
  if(!entry)return;
  if(entry.tier==='long')tier='long';
