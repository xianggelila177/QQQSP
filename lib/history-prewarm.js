...
export function createHistoryPrewarm({history,now=Date.now,enabled=true,statePath='',refreshMs=60000,tickMs=1000,maxSymbols=MAX_WATCHLIST_SYMBOLS,expandAfterMs=5000,canExpand=()=>true,log={warn(){}}}={}){
 ...
 let watchlist=[],persistentWatchlist=[],overflow=[],membershipVersion=0,lastWasExpansion=false;
 ...
 function runDue(){
  ...
  if((!entry||!lastWasExpansion)&&canExpand()&&values.every(e=>e.periods||e.retryAt>now())){
   const expand=values.find(e=>e.periods&&e.tier!=='long'&&e.expandAt<=now()&&!(e.retryAt>now()));if(expand){entry=expand;tier='long';}
  }
  if(!entry)return;
  lastWasExpansion=tier==='long';
