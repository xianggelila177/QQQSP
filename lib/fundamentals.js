...
import {financialNumber,fact} from './financial-values.js';
export {financialNumber,fact};
...
export const regularStatistics=rawRegularStatistics;
...
export function buildBasicMetrics(quote,record=null,{now=Date.now(),ttlMs=21600000,maxAgeMs=604800000,loading=false,enabled=true,failure=null}={}) {
  ...
  if(security&&stats){
    ...
    fields.turnoverAmount=amount==null?absent('no-trade-amount'):fact(amount,{...statInfo,source:stats.amountSource||statInfo.source,asOf:stats.amountAsOf||statInfo.asOf,currency:stats.currency||quote.currency,unit:'money'});
    ...
    if(volume!=null&&denominator!=null){
      ...
      fields.turnoverRate=fact(volume/denominator*100,{...statInfo,unit:'percent',basis,denominator,shareSource:shareInfo.source,shareAsOf:shareInfo.asOf??null,stale:!!(statInfo.stale||shareInfo.stale),calculated:true,formula:'regular-volume / shares * 100'});
  ...
  if(security){
    ...
    if(imbalance?.basis==='full-displayed-book'&&positive(imbalance.asOf)&&financialNumber(imbalance.value)!=null&&Math.abs(imbalance.value)<=100)fields.orderImbalance=fact(imbalance.value,{source:stats.source,asOf:atMs(imbalance.asOf),basis:imbalance.basis,unit:'percent',depth:imbalance.depth??null,calculated:!!imbalance.calculated,formula:imbalance.formula});
  }
  ...
  if(equity&&!floatConflict&&float!=null&&positive(quote.price)!=null&&!fields.floatMarketCap.value){
    const unit=quote.currency?currencyUnitInfo(quote.currency):{currency:null,scale:1};
    fields.floatMarketCap=fact(quote.price*unit.scale*float,{currency:unit.currency,unit:'money',source:quote.src,shareSource:fields.floatShares.source,
      asOf:atMs(quote.quoteAt??quote.ts),shareAsOf:fields.floatShares.asOf??null,basis:'price-times-float',estimated:true,calculated:true,formula:'price * float-shares',stale:!!(quote.stale||fields.floatShares.stale)});
  }
