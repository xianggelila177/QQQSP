...
export function isUsableChartFamily(bars,instrumentType) {
  return Array.isArray(bars)&&bars.length>0&&bars.every(bar=>bar&&typeof bar==='object'&&
    Number.isFinite(bar.t)&&bar.t>0&&validPrice(bar.c,instrumentType)&&
...
export function publishQuote(quote) {
  ...
  const out={...resolvePreviousCloseReference(quote),volume:volumeNumber(quote.volume)};
  if(quote.charts)out.charts=publishCharts(quote.charts);
  if(out.ext&&typeof out.ext==='object')out.ext=Object.fromEntries(Object.entries(out.ext).map(([key,value])=>
