...
async function rawRequest(url, headers = {}, opts = {}, countUrl = null) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = mod.get(url, { agent: isHttp ? undefined : AGENT, headers: { 'User-Agent': UA, ...headers }, ...opts }, (res) => {
      ...
      res.on('end', () => {
        if (settled) return;
        settled = true; clearTimeout(killer);
        const body = Buffer.concat(chunks, bodyBytes).toString(reqEnc);
        const ms = now() - t0;
  ...
  function rawHttpsGet(url,headers={},opts={},countUrl=null){return tracked(options=>rawRequest(url,headers,options,countUrl),opts);}
  function request(url,headers={},opts={}) {return upstream?upstream(url,headers,opts):rawRequest(url,headers,opts);}
  function httpsGet(url,headers={},opts={}){return gate?gate.run(url,signal=>tracked(options=>request(url,headers,options),{...opts,signal}),opts):tracked(options=>request(url,headers,options),opts);}
