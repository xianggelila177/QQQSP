...
async function rawRequest(url, headers = {}, opts = {}, countUrl = null) {
  ...
  function httpsGet(url,headers={},opts={}){
    const {gatePartition,...requestOptions}=opts;
    return gate?gate.run(url,signal=>tracked(options=>request(url,headers,options),{...requestOptions,signal}),{...requestOptions,gatePartition}):tracked(options=>request(url,headers,options),requestOptions);
  }
