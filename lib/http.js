...
export function createHttp(deps,options={}) {
  ...
  async function searchResponse(req,res,url,cors,signal) {
    ...
    const remoteAllowed=!exactLocal||more;
    const searchStarted=Date.now();
    const primary=remoteAllowed&&!futuresQuery&&services.finnhubSearch?
      [['finnhub',true,()=>services.finnhubSearch(yahooTerm||q,{signal})]]:[];
    const primaryResults=await settleWithin(primary.map(([, ,load])=>Promise.resolve().then(load)),Math.min(searchDeadline,4000));
    const finnhubRows=primaryResults[0],needFallback=remoteAllowed&&(more||!Array.isArray(finnhubRows)||!finnhubRows.length);
    ...
    const fallbackResults=await settleWithin(fallback.map(([, ,load])=>Promise.resolve().then(load)),Math.max(1,searchDeadline-(Date.now()-searchStarted)));
    const sources=[...primary,...fallback],results=[...primaryResults,...fallbackResults];
    const bySource=Object.fromEntries(sources.map(([id],i)=>[id,results[i]]));
    const {finnhub:hs,yahoo:ys,tencent:ts,naver:ks,futures:fs}=bySource;
    const fh=Array.isArray(hs)?hs.filter(x=>x&&x.symbol!==idx):[],ya=Array.isArray(ys)?ys.filter(x=>x&&x.symbol!==idx&&(!futuresQuery||x.type==='FUTURE')):[],
      nv=Array.isArray(ks)?ks.filter(x=>x&&x.symbol!==idx):[],tx=Array.isArray(ts)?ts.filter(x=>x&&x.symbol!==idx):[],fu=Array.isArray(fs)?fs.filter(x=>x&&x.symbol!==idx):[];
    const ordered=futuresQuery?[...fu,...ya]:[...fh,...ya,...nv,...tx,...fu];
    const seen=new Set(),exactSymbol=q.toUpperCase();out=out.concat(ordered).map(row=>({...row,symbol:canonicalSearchQuery(row.symbol).toUpperCase()})).filter(x=>symbolValid(x.symbol)&&
      (!(x.type==='INDEX'||x.symbol.startsWith('^'))||marketKeyFor(x.symbol))&&(x.type!=='FUTURE'||futureInstrumentFor(x.symbol))&&!seen.has(x.symbol)&&seen.add(x.symbol))
      .sort((a,b)=>Number(b.symbol===exactSymbol)-Number(a.symbol===exactSymbol)).slice(0,60);
  ...
  async function marketResponse(req,res,url,cors) {
    ...
    const quotes=await Promise.all(parsed.symbols.map(async symbol=>{
      try{return await withDeadline(Promise.resolve().then(()=>services.getCachedQuote(symbol)),requestDeadline);}
      catch(error){if(error.code==='QUOTES_CAPACITY_EXCEEDED')throw error;return {symbol,error:safeError(error.message),code:safeError(error.code||(error.statusCode&&`HTTP_${error.statusCode}`),'QUOTE_UNAVAILABLE')};}
    }));
    record(quotes);
  ...
  async function manualMarketResponse(req,res,url,cors) {
    const expectedOrigin=origin||`http://${req.headers.host}`;
    if(req.headers.origin!==expectedOrigin||req.headers['x-qqqsp-refresh']!=='1'){
      send(req,res,403,{'Content-Type':'application/json',...cors,'Cache-Control':'no-store'},JSON.stringify({code:'REFRESH_ORIGIN_REQUIRED'}));return;
    }
    const parsed=parseSymbols(url.searchParams.get('symbols'),symbols);
    if(parsed.error){send(req,res,400,{'Content-Type':'application/json',...cors},JSON.stringify({error:parsed.error}));return;}
    if(!services.forceRefreshQuotes){send(req,res,503,{'Content-Type':'application/json',...cors},JSON.stringify({code:'SOURCE_REFRESH_UNAVAILABLE'}));return;}
    const waitMs=30000-(now()-lastManualRefreshAt);
    if(waitMs>0){send(req,res,429,{'Content-Type':'application/json',...cors,'Cache-Control':'no-store','Retry-After':String(Math.ceil(waitMs/1000))},JSON.stringify({code:'REFRESH_COOLDOWN'}));return;}
    lastManualRefreshAt=now();
    const result=await services.forceRefreshQuotes(parsed.symbols);
    record(result.quotes);
    send(req,res,200,{'Content-Type':'application/json',...cors,'Cache-Control':'no-store','X-Server-Now-Ms':String(now())},
      JSON.stringify({...result,quotes:applyChartVersions(result.quotes,parseCv(url.searchParams.get('cv')))}));
  }
  const server=http.createServer(async(req,res)=>{
    ...
    try {
      ...
      if(req.method==='POST'&&p==='/api/market/refresh'){
        req.resume();await runRequest(req,res,quoteAdmission,requestDeadline,()=>manualMarketResponse(req,res,url,cors));return;
      }
      ...
      if(p==='/api/stream'&&sse){const parsed=parseSymbols(url.searchParams.get('symbols'),symbols);if(parsed.error){send(req,res,400,{'Content-Type':'application/json'},JSON.stringify({error:parsed.error}));return;}sse.open(req,res,parsed.symbols,url.searchParams.get('cv'),url.searchParams.get('history')!=='off',cors,url.searchParams.get('historyDelta')==='1',url.searchParams.get('regularDelta')==='1');return;}
      ...
      if(p==='/api/chart/detail'&&services.chartDetail){
        await runRequest(req,res,historyAdmission,env.HISTORY_TOTAL_DEADLINE_MS??20000,async signal=>{
          const symbol=canonicalSymbol(url.searchParams.get('symbol'));
          const range=url.searchParams.get('range')||'1d';
          const tapeSession=url.searchParams.get('tapeSession')||'auto';
          if(!symbolValid(symbol)||!['1d','5d'].includes(range)||!['auto','regular','pre','post'].includes(tapeSession)){
            send(req,res,400,{'Content-Type':'application/json',...cors,'Cache-Control':'no-store'},JSON.stringify({code:'BAD_CHART_DETAIL_QUERY'}));return;
          }
          const value=await services.chartDetail.read(symbol,{range,tapeSession,signal});
          send(req,res,200,{'Content-Type':'application/json',...cors,'Cache-Control':'no-store'},JSON.stringify(value));
        });return;
      }
