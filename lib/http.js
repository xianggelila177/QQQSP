import {createMarketContextHttp,CONTEXT_PATH} from './market-context-http.js';
...
export function createHttp(deps,options={}) {
  ...
  const contextHandler=services.marketContext?createMarketContextHttp({service:services.marketContext,apiKey:env.LLM_API_KEY||''}):null;
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    try {
      ...
      const contextDocs={'/api/v1/openapi.json':'market-context.openapi.json','/api/v1/market-context.schema.json':'market-context.schema.json'};
