...
export function createHttp(deps,options={}) {
  ...
  async function searchResponse(req,res,url,cors,signal) {
    ...
    const seen=new Set();out=out.concat(ya,tx).map(row=>({...row,symbol:canonicalSearchQuery(row.symbol).toUpperCase()})).filter(x=>symbolValid(x.symbol)&&(x.type!=='FUTURE'||futureInstrumentFor(x.symbol))&&!seen.has(x.symbol)&&seen.add(x.symbol)).slice(0,60);
