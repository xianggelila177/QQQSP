...
export function createHttp(deps,options={}) {
  ...
  async function searchResponse(req,res,url,cors) {
    ...
    const exactLocal=local.some(x=>x.exact);
    const yahooTerm=/[\u4e00-\u9fff]/.test(q)?services.ALIAS[q.toLowerCase()]||'':q;
    const tasks=exactLocal?[]:[yahooTerm?admission.run(()=>services.yahooSearch(yahooTerm)):Promise.resolve([]),admission.run(()=>services.tencentSuggest(q)),services.koreanSearch?services.koreanSearch(q):Promise.resolve([])];
    const [ys,ts,ks]=await settleWithin(tasks,searchDeadline);
    if(Array.isArray(ks))out=out.concat(ks);
    const ya=Array.isArray(ys)?ys.filter(x=>x&&x.symbol!==idx):[],tx=Array.isArray(ts)?ts.filter(x=>x&&x.symbol!==idx):[];
    const seen=new Set();out=out.concat(ya,tx).filter(x=>symbolValid(x.symbol)&&!seen.has(x.symbol)&&seen.add(x.symbol));
