...
export function publicSourceHealth(value={}) {
  ...
  const streams=v=>v?{...scalar(v,['enabled','status','errorCode','connectionErrorCode','snapshotErrorCode','feed','active','maxSymbols','restRetryAt','restBlocked','nextConnectAt','connectionCheckedAt','connectionHealthy','cacheReadMs']),...(v.primary?{primary:streams(v.primary)}:{}),...(v.backup?{backup:streams(v.backup)}:{})}:null;
  ...
  }))},finnhubRest:{...scalar(value.finnhubRest,['enabled','tokenCount','availableTokens','coolingTokens','nextAvailableAt']),counts:scalar(value.finnhubRest?.counts,['requests','successes','failovers','rateLimits','rejected','failures'])},polling:scalar(value.polling,['mode','primary','pollMs','probeMs','requests','failures','slots','inflight']),
