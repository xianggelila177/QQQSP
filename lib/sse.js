...
export function createSse({engine,historyPrewarm,samples,now=Date.now,heartbeatMs=15000,budget=createStreamBudget()}={}) {
  ...
  function open(req,res,symbols,rawCv,retainHistory=true,cors={}) {
    ...
    let closed=false,pulse,unhistory,unsamples,releaseHistory,unwatch,unsubscribe;
    const cleanup=()=>{if(closed)return;closed=true;clearInterval(pulse);unwatch?.();unsubscribe?.();unhistory?.();unsamples?.();releaseHistory?.();connections.delete(res);budget.release(res);};
    ...
    unsamples=samples?.subscribe(entries=>{const subset=entries.filter(e=>wanted.has(e.symbol));if(subset.length)write('samples',{serverNow:now(),entries:subset});});
    if(samples)write('samples',{serverNow:now(),reset:true,symbols});
