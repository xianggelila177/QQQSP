...
export function createSse({engine,historyPrewarm,samples,now=Date.now,heartbeatMs=15000,budget=createStreamBudget()}={}) {
  ...
  function open(req,res,symbols,rawCv,retainHistory=true,cors={},historyDelta=false,regularDelta=false) {
    ...
    const pending=new Set(),sampleUpdates=new Map(),wanted=new Set(symbols),cv=parseCv(rawCv),historyVersions=new Map();
    const regularSupported=regularDelta||[...cv.values()].some(value=>value.rVer!==undefined);
    try{unwatch=engine.watch(symbols);}catch(error){budget.release(res);res.writeHead(error.statusCode||503,{'Content-Type':'application/json','Retry-After':'5',...cors});res.end(JSON.stringify({code:error.code||'QUOTES_CAPACITY_EXCEEDED',error:error.message}));return;}
    ...
    function pump(){
      ...
      if(kind==='quotes'){
        ...
        if(write('quotes',{serverNow:now(),quotes}))for(const q of quotes)cv.set(q.symbol,{iVer:q.intradayVer,dVer:q.daily30Ver,...(regularSupported?{rVer:q.regularChart?.revision}:{})});
      }else if(kind==='history'){
        const snapshot=historyPrewarm.snapshot([symbol]),next=[];
        const entries=snapshot.entries.map(entry=>({...entry,periods:entry.periods?Object.fromEntries(Object.entries(entry.periods).map(([period,value])=>{
          const key=entry.symbol+':'+period,version=value.seriesId+':'+(value.barsRevision||value.revision);
          next.push([key,version]);return [period,historyDelta&&historyVersions.get(key)===version?{...value,bars:'same'}:value];
        })):entry.periods}));
        if(write('history',{...snapshot,entries}))for(const [key,version] of next)historyVersions.set(key,version);
      }
    ...
    unsubscribe=engine.subscribe(changed=>enqueue('quotes',changed));
