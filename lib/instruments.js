...
export function providerCapabilities(symbol) {
  ...
  const group=['^N225','^SOX'].includes(s)?'index':/^\d{6}\.(KS|KQ)$/.test(s)?'kr':tx?(key==='cn'||key==='hk'||s.startsWith('^')?'other':'us'):null;
  return {marketKey:key,batchGroup:group,batchProviders:group==='index'?['naver-index']:group==='kr'?['naver']:group==='us'?['naver','tencent']:tx?['tencent']:[],tencentCode:tx,legacyQuote:true};
}
