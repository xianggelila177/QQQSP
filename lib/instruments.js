...
const US_INDEX = new Set(['^IXIC','^GSPC','^DJI','^SOX','^RUT','^VIX']);
...
export function instrumentMeta(symbol, meta={}) {
  ...
  const exchangeName=meta.fullExchangeName || meta.exchangeName || (/\.SS$/.test(s)?'SSE':/\.SZ$/.test(s)?'SZSE':/\.BJ$/.test(s)?'BSE':/\.HK$/.test(s)?'HKEX':/\.(T|JP)$/.test(s)?'JPX':/\.(KS|KQ)$/.test(s)?'KRX':MARKET_REGISTRY[marketKeyFor(s)]?.exchange||'');
...
export function providerCapabilities(symbol) {
  ...
  const group=/^\d{6}\.(KS|KQ)$/.test(s)?'kr':tx?(key==='cn'||key==='hk'||s.startsWith('^')?'other':'us'):null;
  return {marketKey:key,batchGroup:group,batchProviders:group==='kr'?['naver']:group==='us'?['naver','tencent']:tx?['tencent']:[],tencentCode:tx,legacyQuote:true};
}
