...
import {twseListingFor} from './providers/twse-listings.js';
...
export function quoteDisplayName(symbol, candidate) {
  const s=String(symbol||'').toUpperCase(),name=typeof candidate==='string'?candidate.trim():'';
  const market=marketKeyFor(s);
  // Naver supplies Korean company names even for US and Japanese listings.
  // Keep Korean listings' own names; use the local catalog or ticker elsewhere.
  if(name&&(market==='kr'||!/\p{Script=Hangul}/u.test(name)))return name;
  const catalog=catalogInstrumentFor(s);
  return (market==='jp'?catalog?.nameJa:null)||catalog?.name||s;
}
...
export function providerCapabilities(symbol) {
  ...
  const group=twseListingFor(s)?'tw':['^N225','^SOX','^GSPC'].includes(s)?'index':/^\d[A-Z0-9]{3}\.T$/.test(s)?'jp':/^\d{6}\.(KS|KQ)$/.test(s)?'kr':tx?(key==='cn'||key==='hk'||s.startsWith('^')?'other':'us'):null;
  return {marketKey:key,batchGroup:group,batchProviders:group==='tw'?['twse']:group==='index'?['naver-index']:group==='jp'?['naver-jp']:group==='kr'?['naver']:group==='us'?['naver','tencent']:tx?['tencent']:[],tencentCode:tx,legacyQuote:true};
}
