...
export function createQuoteSamples({engine,getWatchlist=()=>[],now=Date.now,directory='',enabled=true,tickMs=1000,maxBytes,store:providedStore}={}) {
  ...
  let running=false,initialized=false,timer=null,release=null,unsubscribe=null,membership=[],nextFlush=0,init=null,persistenceStatus='',membershipError=null;
  ...
  function reconcile() {
    const list=[...new Set(getWatchlist().filter(s=>typeof s==='string'&&symbolValid(s)))].slice(0,MAX_WATCHLIST_SYMBOLS);
    if(list.join(',')===membership.join(',')){membershipError=null;return;}
    let next;
    try{next=list.length?engine.watch(list):null;}
    catch(error){membershipError={code:error.code||'SAMPLE_SUBSCRIPTION_FAILED',message:error.message||'采样订阅暂不可用',maxSymbols:error.maxSymbols||null};return;}
    membershipError=null;
    const previous=release;for(const symbol of new Set([...membership,...list]))store.touch(symbol);membership=list;store.retain?.(list);release=next;previous?.();
    for(const symbol of errors.keys())if(!list.includes(symbol))errors.delete(symbol);
  }
  function decorateSnapshot(data) {
    const symbol=data.symbol,collecting=enabled&&running&&membership.includes(symbol);
    const subscriptionError=membershipError&&getWatchlist().includes(symbol)&&!membership.includes(symbol)?membershipError:null;
    const error=!data.supported?data.reason:subscriptionError?.message||errors.get(symbol)||data.capacityError||null;
    return {...data,enabled,collecting,status:!data.supported?'unsupported':subscriptionError?'degraded':!collecting?'paused':error||data.persistenceError?'degraded':data.points.length?'collecting':'warming',error,...(subscriptionError?{errorCode:subscriptionError.code}:{})};
  }
  const snapshot=symbol=>decorateSnapshot(store.snapshot(symbol));
  function runDue() {
  ...
  function emit(){
    const updates=store.takeUpdates().map(decorateSnapshot);
  ...
  return {start,stop,runDue,snapshot,subscribe(fn){listeners.add(fn);return()=>listeners.delete(fn);},
    diagnostics:()=>({enabled,running,initialized,watchlist:[...membership],membershipError,...store.diagnostics()})};
}
