...
export function createQuoteSamples({engine,getWatchlist=()=>[],now=Date.now,directory='',enabled=true,tickMs=1000,maxBytes,store:providedStore}={}) {
  ...
  function reconcile() {
    ...
    const previous=release;for(const symbol of new Set([...membership,...list]))store.touch(symbol);membership=list;store.retain?.(list);release=list.length?engine.watch(list):null;previous?.();
