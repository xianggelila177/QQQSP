...
export function createHistorySource({primary,sina,naver,index,alternative,finnhub,world,twse,getQuote=()=>null,now=Date.now,probeTimeoutMs=3500}={}) {
 ...
 const fetchHistory=async function(symbol,query,options={}) {
  ...
  if(!twseDaily&&sticky?.fn&&sticky.until>now()&&functions.includes(sticky.fn)){functions.splice(functions.indexOf(sticky.fn),1);functions.unshift(sticky.fn);}
  ...
  for(const fn of functions){
   ...
   const failureKey=(fn===twse?'twse':fn===finnhub?'finnhub':fn===world?'world':fn===primary?'primary':fn===backup?'backup':fn===index?'index':fn===naver?'naver':'sina')+':'+key;
   ...
   try{
    ...
    if(fn===twse)break; // The verified official daily source takes precedence over optional Yahoo fallback.
  ...
  throw Object.assign(historyError(conflict?'HISTORY_CLOSE_CONFLICT':'HISTORY_SOURCE_UNAVAILABLE',conflict?'同日历史收盘冲突，等待独立来源确认':'历史来源暂不可用',503),{retryAt:waits.length?Math.min(...waits):now()+60000});
 };
 fetchHistory.close=()=>{closed=true;probes.close();rotation.clear();preferred.clear();alternative?.close?.();finnhub?.close?.();world?.close?.();twse?.close?.();};
 fetchHistory.reopen=()=>{closed=false;probes.reopen();alternative?.reopen?.();finnhub?.reopen?.();world?.reopen?.();twse?.reopen?.();};
 fetchHistory.invalidate=symbol=>{
  for(const key of [...failures.keys()])if(key.endsWith(':'+symbol+':true')||key.endsWith(':'+symbol+':false'))failures.delete(key);
  for(const key of [...preferred.keys()])if(key.startsWith(symbol+':'))preferred.delete(key);
  for(const source of [alternative,finnhub,world,twse,index,naver,primary])source?.invalidate?.(symbol);
 };
 fetchHistory.diagnostics=()=>({probes:probes.diagnostics(),
  ...
  alternative:alternative?.diagnostics?.()||null});
 return fetchHistory;
}
