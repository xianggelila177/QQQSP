...
import {semanticEtag,etagMatches,lifecycleHeaders} from './api-representation.js';
...
export function createMarketAuxRoutes({keys,limiter,engine,advanced,budget,now=Date.now,lifecycle={}}={}){
 ...
 async function handle(req,res,url,cors={}){
  const requestId=randomUUID(),headers={'Content-Type':'application/json; charset=utf-8','Cache-Control':'private, no-cache',Vary:'Authorization, Origin','X-Request-Id':requestId,...lifecycleHeaders(lifecycle),...cors,
