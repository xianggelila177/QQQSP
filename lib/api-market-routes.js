...
export function createMarketAuxRoutes({keys,limiter,engine,advanced,budget,now=Date.now,lifecycle={}}={}){
 ...
 async function handle(req,res,url,cors={}){
  ...
  }catch(e){if(res.headersSent){res.destroy();return;}if(!res.destroyed)fail(/^(?:BAD_CONTEXT_QUERY|UNAUTHORIZED|KEY_ROTATED|KEY_EXPIRED|INSUFFICIENT_SCOPE|MARKET_DATA_NOT_CONFIGURED|SOURCE_[A-Z_]+|RESULT_TOO_LARGE|CONTEXT_[A-Z_]+|QUOTES_CAPACITY_EXCEEDED|CALENDAR_OUTSIDE_COVERAGE|STOPPED)$/.test(e.code||'')?e.code:'SOURCE_UNAVAILABLE',e.statusCode||503);}
