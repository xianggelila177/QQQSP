  ...
  async function getCachedQuote(symbol){
    ...
    if(!pending){
      ...
      pending=Promise.resolve().then(()=>getQuote(symbol)).then(quote=>{
        ...
        const missingHistory=recovery.needsHistory?recovery.needsHistory(symbol,quote?.charts):quote?.charts&&Object.keys(quote.charts).some(key=>!isUsableChartFamily(quote.charts[key]));
        const old=missingHistory?fallback(symbol,null):null;
    ...
    if(recovery.has&&!recovery.has(symbol))return pending;
    let timeout;
    try{return await Promise.race([pending,new Promise((resolve,reject)=>{timeout=setTimeout(()=>{const old=fallback(symbol,null);if(old)resolve(old);else pending.then(resolve,reject);},recoveryWaitMs);})]);}
