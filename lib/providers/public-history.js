...
export function createPublicHistory({httpsGet,now=Date.now,maxEntries=MAX_WATCHLIST_SYMBOLS*2,probeTimeoutMs=3500}={}){
 ...
 const fetch=async(symbol,query,options={})=>{
  ...
  const key=symbol+':'+minute,choice=preferred.get(key);if(choice?.until>now()&&candidates.includes(choice.source)){candidates.splice(candidates.indexOf(choice.source),1);candidates.unshift(choice.source);}
  ...
  for(const source of candidates){
   ...
   if(selected){const baseline=selected;const launched=probes.launch(source+':'+key+':'+query,signal=>load(source,symbol,query,{...options,signal}),{signal:options.signal,
    onSuccess(data){if(broaderHistory(data,baseline)){preferred.delete(key);preferred.set(key,{source,until:now()+300000});while(preferred.size>maxEntries)preferred.delete(preferred.keys().next().value);}}});if(!launched)rotation.retry(key);continue;}
   try{
 ...
 fetch.diagnostics=()=>({probes:probes.diagnostics()});
 return fetch;
}
