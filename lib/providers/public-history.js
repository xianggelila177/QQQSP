...
export function parseNasdaqIntraday(payload,symbol,{now=Date.now()}={}){
 ...
 let bars=sorted(data.chart.map(row=>{const x=number(row.x);return {t:x>1e12?x/1000:x,c:number(row.y),v:null};}).filter(b=>b.t>0&&b.t*1000<=now&&b.c>0));
 ...
 return toChart(symbol,bars,{source:'nasdaq-intraday',minute:true});
}
...
export function parseEastmoneyHistory(payload,symbol,secid,{now=Date.now()}={}){
 ...
 const market=marketKeyFor(symbol),zone=market==='cn'?'Asia/Shanghai':market==='hk'?'Asia/Hong_Kong':'America/New_York';
 ...
 return toChart(symbol,bars,{source:'eastmoney-history',zone,currency:market==='cn'?'CNY':market==='hk'?'HKD':'USD',exchange:market==='cn'?(symbol.endsWith('.SS')?'SHH':'SHZ'):market==='hk'?'HKG':'US'});
}
