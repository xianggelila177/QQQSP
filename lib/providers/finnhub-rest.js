...
export function createFinnhubRestPool({ httpsGet, tokens = [], now = Date.now, timeout = 8000 } = {}) {
  ...
  function cooldown(slot, response, at) {
    ...
    // A 403 can deny one paid dataset while the same token still serves quotes.
    // Let that dataset's adapter cool down without disabling the whole pool.
    if (response.status === 403) { counts.rejected++; return null; }
  ...
  async function request(path, { signal, timeout: requestTimeout = timeout } = {}) {
    ...
    const tried = new Set();
    let lastError = null;
    let denied = 0, deniedResponse = null;
    while (tried.size < slots.length) {
      ...
      if(response.status===403){
        denied++;deniedResponse=response;
        if(tried.size<slots.length)counts.failovers++;
        continue;
      }
      if (!code) {
    ...
    if(denied===slots.length)return deniedResponse;
