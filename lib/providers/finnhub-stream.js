...
export function createFinnhubProvider({token='', maxSymbols=30, now=Date.now,
  newSocket=url=>new WebSocket(url), schedule=setInterval, cancel=clearInterval, random=Math.random,onTrade=()=>{}}={}) {
  ...
  let socket=null, timer=null, running=false, generation=0, tradeCursor=0, status=token?'idle':'disabled';
  ...
  function receive(data) {
    ...
    for(const item of message.data) {
      ...
      if(!item || !sent.has(symbol) || item.s!==wireSymbol(symbol) || typeof item.p!=='number' || !Number.isFinite(item.p) || item.p<=0 ||
        !Number.isSafeInteger(item.t)||item.t<=0||item.t>now()+1000) {counts.rejectedTrades++;continue;}
      const size=typeof item.v==='number'&&Number.isFinite(item.v)&&item.v>0?item.v:null;
      if(size!==null)onTrade({symbol,source:'finnhub',price:item.p,size,at:item.t,id:item.i??null,
        connectionCursor:generation+':'+(++tradeCursor),exchange:null});
      ...
      if(old&&item.t<old.quoteAt)continue;
