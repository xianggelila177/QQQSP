...
export function tencentFinancials(p,{symbol,instrumentType,currency,quoteAt,now}) {
  ...
  // Tencent's own adaptHS/adaptUS maps zgb/ltgb, week52 and lyr_ratio:
  // https://st.gtimg.com/quotes/hs-fund/bundle.13362df9.js
  put('sharesOutstanding',f[cn?73:62],{unit:'shares'});
  put('floatShares',f[cn?72:63],{unit:'shares'});
  if(p.week52High>0&&p.week52Low>0&&p.week52High>=p.week52Low){
    put('week52High',p.week52High,{currency,unit:'price',basis:'provider-52-week-range'});
    put('week52Low',p.week52Low,{currency,unit:'price',basis:'provider-52-week-range'});
  }
  if(instrumentType==='EQUITY'){
    put('priceToBook',f[cn?46:51],{unit:'ratio',basis:'latest-book',asOf:quoteAt});
    put('peLYR',f[cn?53:41],{unit:'ratio',basis:'last-fiscal-year',asOf:quoteAt});
