...
import { instrumentTypeFor, instrumentMeta, classifyMarket, quoteDisplayName, tencentCodeFor, validWeek52Range } from '../instruments.js';
...
import {createNaverIdentityResolver,matchesNaverIdentity} from './naver-identity.js';
import {regularSnapshotReferenceAt} from '../previous-close-reference.js';
...
export function createBatchProvider({ now = () => Date.now(), httpsGet, indexProvider, fallbackQuote, pollMs=2000 } = {}) {
  ...
  const knownIdentity = symbol => {
    const item=catalogInstrumentFor(symbol),code=item?.naverCode;
    // Catalog routes carry explicit provider evidence. Tencent suffixes do not.
    if(item?.market==='us'&&typeof code==='string'&&/^[A-Z][A-Z0-9-]*(?:\.[ONK])?$/.test(code)){
      const exchange={NASDAQ:'NSQ',NYSE:'NYS',AMEX:'AMX'}[item.exchange]||
        ({O:'NSQ',N:'NYS',K:'AMX'})[code.split('.')[1]]||null;
      return {code,exchange,instrumentType:item.type,provenance:'catalog'};
    }
    if(symbol==='SPY')return {code:'SPY',exchange:null,instrumentType:'ETF',provenance:'verified-static'};
    if(KNOWN_NASDAQ.has(symbol))return {code:symbol+'.O',exchange:null,instrumentType:instrumentTypeFor(symbol),provenance:'verified-static'};
    return null;
  };
  ...
  const identities=createNaverIdentityResolver({request,now});
  const rejected=(symbol,identity)=>{
    const entry=rejectedIdentities.get(symbol);
    if(entry&&entry.until<=now()){rejectedIdentities.delete(symbol);return false;}
    return !!entry&&entry.code===identity?.code;
  };
  const descriptor=symbol=>{
    const verified=identities.peek(symbol),known=knownIdentity(symbol);
    return [verified,known].find(identity=>identity&&!rejected(symbol,identity))||null;
  };
  function invalidateNaverIdentity(symbol,identity,reason='NAVER_IDENTITY_CONFLICT'){
    if(!identity?.code)return;
    identities.invalidate(symbol,identity.code);
    rejectedIdentities.set(symbol,{code:identity.code,reason,until:now()+60000});
    while(rejectedIdentities.size>200)rejectedIdentities.delete(rejectedIdentities.keys().next().value);
  }
  ...
  function parseNaverQuote(row, symbol, pollAfterMs, identity) {
    const japanese=/^\d[A-Z0-9]{3}\.T$/.test(symbol);
    if (!row || (japanese?row.reutersCode!==symbol||row.symbolCode!==symbol.slice(0,-2):symbol.endsWith('.KS') || symbol.endsWith('.KQ') ? row.itemCode !== symbol.slice(0,6) : row.symbolCode !== symbol)) return null;
    const exchange = row.stockExchangeType || {};
    if(identity?.exchange&&!matchesNaverIdentity(row,symbol,identity))return null;
    // The same numeric code exists at other venues. All identity dimensions
    // must agree; neither a matching company name nor a positive price suffices.
    if(japanese&&(exchange.code!=='TYO'||exchange.zoneId!=='Asia/Tokyo'||exchange.nationType!=='JPN'||row.currencyType?.code!=='JPY'))return null;
    ...
    const useOver = !japanese && overAt > regularAt && overAt <= now()+5000 && overPrice > 0 && ['PRE_MARKET','AFTER_MARKET'].includes(over?.tradingSessionType);
    ...
    const suppliedType=identity?.instrumentType || row.instrumentType || row.quoteType || row.stockType || row.itemType;
    const instrumentType = instrumentTypeFor(symbol, suppliedType);
    ...
    return common(symbol, {price, quoteAt, ts:quoteAt, regularMarketPrice:regular, regularPrice:regular, regularQuoteAt:regularSession==='REGULAR'?regularAt:null, priceSession,
      prevClose,providerPrevClose:prevClose,regularPreviousClose:regularSession==='REGULAR'?prevClose:null,regularPreviousCloseQuoteAt:regularSession==='REGULAR'?regularAt:null,
      change:prevClose == null ? null : price-prevClose,changePct:prevClose ? (price/prevClose-1)*100 : null,
      open,dayHigh,dayLow,volume,volumeUnit:'shares',turnoverAmount:volumeNumber(row.accumulatedTradingValueRaw),financials,
      name:exchange.nameEng || '',displayName:quoteDisplayName(symbol,(japanese?catalogInstrumentFor(symbol)?.nameJa:null)||row.stockName),exchangeName:exchange.nameEng || '',
      currency:row.currencyType?.code || (symbol.endsWith('.KS') || symbol.endsWith('.KQ') ? 'KRW':'USD'), instrumentType,instrumentTypeSource:instrumentMeta(symbol,{instrumentType:suppliedType}).instrumentTypeSource,
      ...
      src:japanese?'naver-jp':/\.(KS|KQ)$/.test(symbol) ? 'naver-kr':'naver-us',feedDelayMinutes:number(exchange.delayTime),
      feedDelaySource:number(exchange.delayTime)!=null?'naver-exchange-delay':null,isDelayed:number(exchange.delayTime)==null?null:number(exchange.delayTime)>0,pollAfterMs,
  ...
  function parseTencentBatch(body, symbols) {
    ...
    for (const [,code,line] of decodeGbkSmart(body).matchAll(/v_([A-Za-z0-9.\-]+)="([^"]*)"/g)) {
      ...
      // An opening-time placeholder can accompany yesterday's close before
      // the bell. A future source clock is not a newer reported trade.
      if (!(p.price>0)||!ts||ts>now()+5000)continue;
      ...
      const sourceSession=priceSessionFor(symbol,ts),priceSession=cn?'REGULAR':sourceSession;
      const regularQuoteAt=regularSnapshotReferenceAt(symbol,ts,{allowPublishedClose:cn});
      const quote=common(symbol,{price:p.price,quoteAt:ts,ts,priceSession,regularPrice:regularQuoteAt?p.price:null,regularQuoteAt,
        ...(cn&&sourceSession!=='REGULAR'&&regularQuoteAt?{quoteTimeBasis:'provider-published'}:{}),prevClose:p.prevClose,change:p.prevClose == null?null:p.price-p.prevClose,
      ...
      if(cn){
        quote.orderBook=normalizeOrderBook({symbol,currency,source:'tx-batch',coverage:'provider-top-of-book',asOf:ts,checkedAt:now(),sizeUnit:'lots',
          bid:{price:volumeNumber(p.raw[9]),size:volumeNumber(p.raw[10])},ask:{price:volumeNumber(p.raw[19]),size:volumeNumber(p.raw[20])}},quote,now());
  ...
  async function fetchSnapshotBatch(symbols,{group,signal}={}) {
    signal?.throwIfAborted();
    ...
    if(group==='index') {
      ...
      if(fallbackQuote)for(const symbol of syms){
        if(!['^N225','^SOX','^GSPC'].includes(symbol)||quotes.get(symbol)?.price>0&&!quotes.get(symbol)?.error)continue;
        try{
    ...
    if(group==='jp') {
      const japanese=syms.filter(s=>/^\d[A-Z0-9]{3}\.T$/.test(s));
      if(!japanese.length)return {quotes:[],pollAfterMs:70000};
      const result=await request(NAVER+'/worldstock/stock/'+japanese.join(','));
      const data=JSON.parse(result.body),pollAfterMs=interval(data.pollingInterval);
      const rows=new Map((data.datas||[]).map(row=>[row.reutersCode,row]));
      return {quotes:japanese.map(symbol=>parseNaverQuote(rows.get(symbol),symbol,pollAfterMs)).filter(Boolean),pollAfterMs};
    }
    if(group==='kr') {
    ...
    const unknown=syms.filter(s=>!descriptor(s));
    let fallback=[];
    if(unknown.length) {try{fallback=await tencent(unknown,{signal});}catch(error){if(signal?.aborted||error.code==='STOPPED')throw error;/* Naver-known siblings can still refresh */}}
    const unresolved=unknown.filter(s=>!descriptor(s));
    const discovery=Promise.all(unresolved.map(s=>identities.resolve(s,{signal})));
    // A newly added ETF must not hold already resolved siblings behind a search
    // request. Only an all-unknown cold batch waits for its bounded discovery.
    if(syms.some(s=>descriptor(s)))void discovery.catch(()=>{});else await discovery;
    signal?.throwIfAborted();
    const mapped=syms.map(symbol=>({symbol,identity:descriptor(symbol)})).filter(x=>x.identity);
    let primary=[],pollAfterMs=2000,primaryError=null;
    const identityIssues={};
    if(mapped.length) {
      try {
        const response=await request(NAVER+'/worldstock/stock/'+mapped.map(x=>x.identity.code).join(','),'utf8',signal);
        const data=JSON.parse(response.body);pollAfterMs=interval(data.pollingInterval);
        if(!Array.isArray(data.datas))throw Object.assign(new Error('Naver snapshot missing datas'),{code:'NAVER_BAD_RESPONSE'});
        const rows=new Map(data.datas.map(r=>[r.reutersCode,r]));
        for(const {symbol,identity} of mapped){
          const row=rows.get(identity.code);
          if(!row){
            identityIssues[symbol]='NAVER_NO_DATA';invalidateNaverIdentity(symbol,identity,'NAVER_NO_DATA');
            void identities.resolve(symbol,{signal}).catch(()=>{});continue;
          }
          if(identity.exchange&&!matchesNaverIdentity(row,symbol,identity)){
            identityIssues[symbol]='NAVER_IDENTITY_CONFLICT';invalidateNaverIdentity(symbol,identity);
            void identities.resolve(symbol,{signal}).catch(()=>{});continue;
          }
          const quote=parseNaverQuote(row,symbol,pollAfterMs,identity);
          if(quote)primary.push(quote);else identityIssues[symbol]='NAVER_QUOTE_UNUSABLE';
        }
      }catch(error){if(signal?.aborted||error.code==='STOPPED')throw error;primaryError=error;pollAfterMs=Math.max(10000,error.retryAfterMs||0);}
    }
    const got=new Set([...primary,...fallback].map(q=>q.symbol));
    const missing=syms.filter(s=>!got.has(s));
    if(missing.length) {try{fallback.push(...await tencent(missing,{signal}));}catch(error){if(signal?.aborted||error.code==='STOPPED')throw error;if(!primary.length&&!fallback.length)throw primaryError||error;}}
    ...
    return {quotes:[...quotes.values()],pollAfterMs:Math.max(pollAfterMs,...[...quotes.values()].filter(q=>q.src==='tx-batch').map(q=>q.pollAfterMs)),identityIssues};
  }
  async function resolveNaverIdentity(symbol,{signal}={}){
    if(!/^[A-Z][A-Z0-9-]{0,15}$/.test(symbol))return null;
    return descriptor(symbol)||await identities.resolve(symbol,{signal}).then(identity=>
      identity&&!rejected(symbol,identity)?identity:null);
  }
  async function resolveNaverCode(symbol,options){return (await resolveNaverIdentity(symbol,options))?.code||null;}
  return { parseNaverQuote, parseTencentBatch, fetchSnapshotBatch, tencent,resolveNaverIdentity,resolveNaverCode,invalidateNaverIdentity,
    close(){identities.close();rejectedIdentities.clear();},reopen(){identities.reopen();} };
}
