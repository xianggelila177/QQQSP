...
export function createBatchProvider({ now = () => Date.now(), httpsGet, pollMs=2000 } = {}) {
  ...
  const knownCode = symbol => symbol === 'SPY' ? 'SPY' : KNOWN_NASDAQ.has(symbol) ? symbol+'.O' : null;
  ...
  async function request(url, encoding = 'utf8') {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 3500);
    try {
      const response = await httpsGet(url, { Referer: 'https://m.stock.naver.com/' }, { encoding, signal:controller.signal,timeout:3500 });
  ...
  function parseNaverQuote(row, symbol, pollAfterMs) {
    ...
    return common(symbol, {price, quoteAt, ts:quoteAt, regularMarketPrice:regular, regularPrice:regular, regularQuoteAt:regularAt, priceSession,
      prevClose,change:prevClose == null ? null : price-prevClose,changePct:prevClose ? (price/prevClose-1)*100 : null,
      open,dayHigh,dayLow,volume,volumeUnit:'shares',
  ...
  function parseTencentBatch(body, symbols) {
    ...
    for (const [,code,line] of decodeGbkSmart(body).matchAll(/v_([A-Za-z0-9.\-]+)="([^"]*)"/g)) {
      ...
      const typeHint=symbol.startsWith('^')?'INDEX':p.raw.includes('GP-ETF')?'ETF':'';
      ...
      const quote=common(symbol,{price:p.price,quoteAt:ts,ts,priceSession,regularPrice:priceSession==='REGULAR'?p.price:null,prevClose:p.prevClose,change:p.prevClose == null?null:p.price-p.prevClose,
        changePct:p.prevClose ? (p.price/p.prevClose-1)*100:null,open:p.open,dayHigh:p.high,dayLow:p.low,volume:p.volume,
  ...
  async function tencent(symbols) {
    const codes=symbols.map(tencentCode).filter(Boolean);
    if(!codes.length)return [];
    const result=await request('https://qt.gtimg.cn/q='+codes.join(','),'latin1');
  ...
  return { parseNaverQuote, parseTencentBatch, fetchSnapshotBatch, tencent };
}
