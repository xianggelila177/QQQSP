...
export function createBatchProvider({ now = () => Date.now(), httpsGet, indexProvider, fallbackQuote, pollMs=2000 } = {}) {
  ...
  const knownCode = symbol => {
    ...
    if(item?.market==='us'&&typeof code==='string'&&/^[A-Z][A-Z0-9-]*(?:\.[ONK])?$/.test(code))return code;
  ...
  const descriptor=symbol=>knownCode(symbol)?{code:knownCode(symbol)}:identities.peek(symbol)||
    (descriptors.get(symbol)?.at>now()-86400000?descriptors.get(symbol):null);
  ...
  function parseNaverQuote(row, symbol, pollAfterMs, identity) {
    ...
    return common(symbol, {price, quoteAt, ts:quoteAt, regularMarketPrice:regular, regularPrice:regular, regularQuoteAt:regularSession==='REGULAR'?regularAt:null, priceSession,
      prevClose,providerPrevClose:prevClose,change:prevClose == null ? null : price-prevClose,changePct:prevClose ? (price/prevClose-1)*100 : null,
  ...
  async function fetchSnapshotBatch(symbols,{group,signal}={}) {
    ...
    if(mapped.length) {
      try {
        ...
        primary=mapped.map(x=>parseNaverQuote(rows.get(x.identity.code),x.symbol,pollAfterMs,x.identity)).filter(Boolean);
  ...
  async function resolveNaverCode(symbol,{signal}={}){
    let code=descriptor(symbol)?.code;
    if(!code&&/^[A-Z][A-Z0-9-]{0,15}$/.test(symbol)){
      try{await tencent([symbol],{signal});}catch(error){if(signal?.aborted||error.code==='STOPPED')throw error;}
      code=descriptor(symbol)?.code||(await identities.resolve(symbol,{signal}))?.code;
    }
    return code||null;
  }
  return { parseNaverQuote, parseTencentBatch, fetchSnapshotBatch, tencent,resolveNaverCode,
    close(){identities.close();descriptors.clear();},reopen(){identities.reopen();} };
}
