...
export function createFinancialSources({httpsGet,batch,fetchYahooSummary,finnhubToken='',now=Date.now,statementTtlMs=21600000}={}){
  ...
  const sources=[
    {id:'tencent-financial',priority:10,priorities:{peLYR:35},ttlMs:60000,fields:['priceToBook','peLYR','marketCap','floatMarketCap','sharesOutstanding','floatShares','week52High','week52Low'],match:q=>!q.symbol.endsWith('.HK')&&!!tencentCodeFor(q.symbol),load:async(symbol,{signal})=>{
      ...
      if(!q?.financials)throw unavailable('FUNDAMENTALS_EMPTY');return {...q.financials,statistics:regularTradingStatistics(q)};
    }},
  ...
  if(finnhubToken)sources.push({id:'finnhub-metric',priority:15,ttlMs:statementTtlMs,lane:'slow',fields:['priceToBook','peTTM','peLYR','psTTM'],match:q=>us(q)&&q.instrumentType==='EQUITY',load:async(symbol,{signal})=>normalizeFinnhubFundamentals(symbol,await request('https://finnhub.io/api/v1/stock/metric?symbol='+encodeURIComponent(symbol)+'&metric=all',{signal,headers:{'X-Finnhub-Token':finnhubToken}}),{now:now()})});
  return sources;
}
