...
export function createFinancialSources({httpsGet,batch,fetchYahooSummary,finnhubRequest=null,finnhubToken='',now=Date.now,statementTtlMs=21600000}={}){
  ...
  async function basic(symbol,{signal}={}){
    const identity=batch.resolveNaverIdentity?
      await batch.resolveNaverIdentity(symbol,{signal}):
      {code:await batch.resolveNaverCode(symbol,{signal})};
    const code=identity?.code;if(!code)throw unavailable('FUNDAMENTALS_UNSUPPORTED');
    const cached=basicCache.get(symbol);
    if(cached?.code===code&&cached.exchange===identity.exchange&&
      cached.instrumentType===identity.instrumentType&&now()-cached.at<60000)return cached.data;
    return basicTasks.run(symbol+':'+code,async taskSignal=>{
      const data=await request('https://api.stock.naver.com/stock/'+encodeURIComponent(code)+'/basic',{signal:taskSignal});
      if(data.symbolCode!==symbol||data.reutersCode!==code||
        identity.exchange&&!matchesNaverIdentity(data,symbol,identity)){
        batch.invalidateNaverIdentity?.(symbol,identity,'NAVER_FINANCIAL_IDENTITY_CONFLICT');
        throw unavailable('FUNDAMENTALS_IDENTITY');
      }
      basicCache.set(symbol,{at:now(),code,exchange:identity.exchange,instrumentType:identity.instrumentType,data});
      while(basicCache.size>128)basicCache.delete(basicCache.keys().next().value);return data;
    },{signal});
  }
  const sources=[
    {id:'tencent-financial',priority:20,priorities:{peLYR:35},ttlMs:60000,fields:['priceToBook','peLYR','marketCap','floatMarketCap','sharesOutstanding','floatShares','week52High','week52Low'],match:q=>!q.symbol.endsWith('.HK')&&!!tencentCodeFor(q.symbol),load:async(symbol,{signal})=>{
    ...
    {id:'naver-basic',priority:60,ttlMs:60000,fields:['priceToBook','marketCap','sharesOutstanding','bookValue','week52High','week52Low'],match:q=>us(q)||kr(q),load:async(symbol,{signal})=>{
    ...
    {id:'naver-company',priority:60,ttlMs:statementTtlMs,fields:['sharesOutstanding','trailingEps','annualEps'],match:kr,load:async(symbol,{signal})=>normalizeNaverCompanyFinancials(symbol,await request('https://navercomp.wisereport.co.kr/v2/company/c1010001.aspx?cmp_cd='+symbol.slice(0,6),{signal,text:true}),{now:now()})},
    {id:'yahoo-summary',priority:70,ttlMs:statementTtlMs,lane:'slow',fields:['priceToBook','peTTM','peLYR','psTTM','marketCap','sharesOutstanding','floatShares','dividendTTM','dividendYieldTTM','week52High','week52Low'],load:async(symbol,{signal})=>normalizeYahooFundamentals(symbol,await fetchYahooSummary(symbol,{signal}),{now:now()})},
    {id:'naver-statements',priority:60,ttlMs:statementTtlMs,lane:'slow',fields:['trailingEps','annualEps','revenueTTM','peLYR'],match:q=>us(q)&&q.instrumentType==='EQUITY',load:async(symbol,{signal})=>{
