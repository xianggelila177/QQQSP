...
const naverExchange={NSQ:'NASDAQ',NYS:'NYSE',AMX:'AMEX'};
...
export function createNaverWorldHistory({httpsGet,resolveCode,invalidateIdentity,now=Date.now}={}){
  ...
  const fetch=async(symbol,query,{signal}={})=>{
    ...
    const resolved=await resolveCode(symbol,{signal});
    const identity=typeof resolved==='string'?{code:resolved}:resolved,code=identity?.code;
    if(!code||!/^[A-Z][A-Z0-9-]{0,15}(?:\.[A-Z])?$/.test(code)||
      identity.exchange&&!naverExchange[identity.exchange])
      throw historyError('HISTORY_IDENTITY_UNRESOLVED','无法核验 Naver 证券代码',422);
    const identityKey=[symbol,code,identity.exchange||''].join(':'),key=[identityKey,resolution,start,end].join(':');
    const failureKey=identityKey+':'+resolution,failed=failures.get(failureKey),old=cache.get(key);
    ...
    const job=(async()=>{
      ...
      for(const exchange of identity.exchange?[naverExchange[identity.exchange]]:exchanges(code)){
        ...
        for(let from=start;from<end;from+=CHUNK){
          ...
          let bars;
          try{bars=parseNaverWorldCandles(payload,symbol,code,exchange,resolution,{now:now()});}
          catch(error){
            if(error.code==='HISTORY_IDENTITY_CONFLICT'&&identity.exchange)
              invalidateIdentity?.(symbol,identity,'NAVER_HISTORY_IDENTITY_CONFLICT');
            throw error;
          }
        ...
        const data={source:'naver-world-chart',retrievalLimited:true,
          meta:{symbol,currency:'USD',exchangeName:exchange,exchangeTimezoneName:'America/New_York',
            instrumentType:identity.instrumentType||instrumentTypeFor(symbol),identityProvenance:identity.provenance||null,
            // Verified against the source's one-minute candles: each timestamp
            // starts [t, t + interval), including the mixed 16:00 five-minute bar.
        ...
        cache.set(key,{data,until:now()+60000});failures.delete(failureKey);
        while(cache.size>80)cache.delete(cache.keys().next().value);
        return data;
      }
      throw lastError;
    })().catch(error=>{
      if(!signal?.aborted&&error.code!=='STOPPED')failures.set(failureKey,
