...
export function createMacroQuoteReader({httpsGet,futures,fetchChart,now=Date.now,candidateTimeoutMs=6500}={}){
 const failures=new Map(),preferred=new Map(),lastGood=new Map(),tasks=new Map();let closed=false;
 ...
 function bounded(id,load){
  ...
  let timer;return Promise.race([job,new Promise((_,reject)=>{timer=setTimeout(()=>reject(Object.assign(new Error('候选来源超时'),{code:'MACRO_SOURCE_TIMEOUT'})),candidateTimeoutMs);})]).finally(()=>clearTimeout(timer));
 }
 function quality(q){
 ...
 async function readQuote(symbol){
  ...
  const ordered=choice&&choice.quality>=5&&now()<choice.until?[...candidates.filter(c=>c[0]===choice.id),...candidates.filter(c=>c[0]!==choice.id)]:candidates;
  ...
  for(const [id,load]of ordered){
   ...
   try{
    const q=await bounded(id,load);if(!quality(q))throw missing('来源未返回有效价格');
    failures.delete(id);const score=consider(q,id);
    diagnostic.push({source:id,status:score>=5?'selected':score===4?'observation':score===3?'delayed':score===2?'daily':'stale',checkedAt:q.sourceCheckedAt});
    if(score>=5)break;
  ...
  const previous=lastGood.get(symbol);if(previous)consider(previous.q,previous.id);
  if(best){
 ...
 return {readQuote,close(){closed=true;sina.close();yields.close();daily.close();},reopen(){closed=false;sina.reopen();yields.reopen();daily.reopen();},diagnostics:()=>({failures:Object.fromEntries(failures),selected:Object.fromEntries(preferred),inflight:tasks.size})};
}
