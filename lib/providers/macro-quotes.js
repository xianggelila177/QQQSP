...
export function createMacroQuoteReader({httpsGet,futures,fetchChart,publicSources=null,now=Date.now,candidateTimeoutMs=6500}={}){
 ...
 const rotation=createSourcePolling({now,intervalMs:30000,maxEntries:16});
 ...
 function quality(q){
  if(!validPrice(q?.price,q?.instrumentType))return 0;
 ...
 async function readQuote(symbol){
  ...
  const probe=rotation.take(symbol,candidates.filter(([id])=>id!==ordered[0][0]).map(([id])=>id));
  ...
  for(const [id,load]of ordered){
   if(best?.score>=4&&id!==probe)continue;
   if(best?.score>=3.5&&id!==probe&&(id.startsWith('fred:')||id==='sina:US10YT'))continue;
