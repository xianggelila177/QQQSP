...
export function createKoreanSearch({httpsGet,now=Date.now}={}) {
  const cache=createCachedResource({now,ttlMs:3600000,retryMs:60000,maxEntries:32,loader:async (query,{signal})=>{
    const match=/^(\d{6})(?:\.(KS|KQ))?$/.exec(query);
    if(!match)return [];
    const response=await httpsGet('https://polling.finance.naver.com/api/realtime/domestic/stock/'+match[1],{Referer:'https://m.stock.naver.com/'},{timeout:2500,signal});
  ...
  return (query,options)=>/^\d{6}(?:\.(KS|KQ))?$/.test(String(query).toUpperCase())?cache.get(String(query).toUpperCase(),options):Promise.resolve([]);
}
