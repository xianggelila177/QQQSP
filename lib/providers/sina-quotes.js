...
export function parseSinaQuotes(body,symbols,{now=Date.now(),pollMs=1000}={}){
 ...
 for(const [,code,text] of decodeGbkSmart(body).matchAll(/(?:var\s+)?hq_str_([a-zA-Z0-9_]+)\s*=\s*"([^"\r\n]*)"\s*;?/g)){
  ...
  out.push(publishQuote({symbol,...meta,displayName:hk?f[1]:f[0],name:meta.exchangeName,currency:us?'USD':hk?'HKD':'CNY',
   ...
   prevClose:prevClose>0?prevClose:null,change:prevClose>0?price-prevClose:null,changePct:prevClose>0?(price/prevClose-1)*100:null,
   open,dayHigh:high,dayLow:low,volume:n(f[us?10:hk?12:8]),volumeUnit:'shares',ohlcSession:'REGULAR',ohlcConsistent:priceSession==='REGULAR'&&low!=null&&high!=null&&low<=price&&price<=high,
