...
export function createFastPolling({httpsGet,legacy,now=Date.now,pollMs=1000,preferred='sina'}={}) {
  ...
  function pick(best,rows){for(const q of rows){const prior=best.get(q.symbol);if(!prior||q.quoteAt>prior.quoteAt||q.quoteAt===prior.quoteAt&&q.sourceCheckedAt>prior.sourceCheckedAt)best.set(q.symbol,q);}}
  ...
  return {
    async fetchSnapshotBatch(symbols,{group}={}) {
      ...
      return {quotes,pollAfterMs:pollMs};
    },
