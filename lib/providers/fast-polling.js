...
export function createFastPolling({httpsGet,legacy,finnhubQuotes=null,now=Date.now,pollMs=1000,preferred='sina',probeMs=30000}={}) {
  ...
  return {
    async fetchSnapshotBatch(symbols,{group}={}) {
      ...
      if(finnhubQuotes?primary.length===symbols.length&&primary.every(q=>!old(q)):primary.some(q=>!old(q)))
