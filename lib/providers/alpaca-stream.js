...
// Canonical US share classes use '-' locally and '.' on Alpaca's data API.
...
export function createAlpacaProvider({env = {}, now = Date.now, httpsGet,
  ...
  schedule = setInterval, cancel = clearInterval,onTrade=()=>{},onTradeInvalidation=()=>{}} = {}) {
  const config = alpacaConfig(env);
  const active = new Map(), records = new Map();
  let running = false, generation = 0, timer = null, socket = null;
  let authenticated = false, connectingAt = 0, connectedAt = 0, framesAt=null, probeAt=null;
  let nextConnectAt = 0, reconnects = 0, restUntil = 0, restFailures = 0, lastRestAt = -Infinity;
  let restJob = null, status = config.enabled ? 'idle' : 'disabled', errorCode = null, restErrorCode = null;
  ...
  function reconcile() {
    ...
    if (remove.length && send({action:'unsubscribe', trades:remove.map(wireSymbol),quotes:remove.map(wireSymbol)})) remove.forEach(symbol => requested.delete(symbol));
    if (add.length && send({action:'subscribe', trades:add.map(wireSymbol),quotes:add.map(wireSymbol)})) add.forEach(symbol => requested.add(symbol));
  }
  ...
  function acceptTrade(message) {
    const symbol=canonicalSymbol(message.S);
    ...
    onTrade({symbol,source:'alpaca-'+config.feed,price,size:message.s,at:stamp.milliseconds,id,
      connectionCursor:generation+':'+id,exchange:message.x});
    const record = recordFor(symbol), previous = record.trade;
    ...
    record.trade = Object.freeze({symbol, price, quoteAt:stamp.milliseconds, sourceTimestamp:message.t,
    ...
    counts.receivedTrades++;notify(symbol);return true;
  }
  function acceptBook(message) {
    const symbol=canonicalSymbol(message.S);
    if (!active.has(symbol)) return false;
    const stamp=tradeTimestamp(message.t),record=recordFor(symbol);
    if (!stamp || record.bookOrder!==null&&stamp.order<record.bookOrder) return false;
    const book=alpacaOrderBook(symbol,message,{source:'alpaca-'+config.feed,coverage:config.feed==='iex'?'single-exchange':'us-sip',checkedAt:now(),now:now()});
    if (!book) return false;
    record.book=book;record.bookOrder=stamp.order;notify(symbol);return true;
  }
  function invalidate(message) {
    const symbol=canonicalSymbol(message.S),record = records.get(symbol), previous = record?.trade;
    const id = message.T === 'c' ? message.oi : message.i;
    if(active.has(symbol))onTradeInvalidation({symbol,source:'alpaca-'+config.feed,id:id==null?null:String(id),exchange:message.x,kind:message.T==='c'?'correction':'cancellation'});
    if (previous && String(id) === previous.id && message.x === previous.exchange) {
      ...
      record.nextSnapshotAt = 0; counts.invalidations++;notify(symbol);
    }
  }
  function receive(data) {
    ...
    for (const message of batch) {
      ...
      if (message.T === 'success' && message.msg === 'authenticated') {
        ...
        if(!Array.isArray(message.trades)){fail('INVALID_SUBSCRIPTION');return;}
        acknowledged.clear();for(const value of message.trades){const symbol=canonicalSymbol(value);if(active.has(symbol))acknowledged.add(symbol);}
        probeAt=null;
        status=subscriptionBlocked?'subscription-limited':acknowledged.size===active.size?'streaming':'subscribing';
      } else if (message.T === 'error') {
        const code = Number(message.code);
        if (code === 405) {
          subscriptionBlocked = true; status = 'subscription-limited'; errorCode = 'SYMBOL_LIMIT';probeAt=null;
          // The provider preserves subscriptions from before the rejected request.
          requested.clear();for(const symbol of acknowledged)requested.add(symbol);
        }
  ...
  function connect() {
    ...
    try {
      ...
      socket = ws; connectingAt = now(); framesAt=null;probeAt=null;status = 'connecting';
  ...
  async function refreshSnapshots(symbols, epoch, job) {
    ...
    let deadline;
    try {
      const request = httpsGet('https://data.alpaca.markets/v2/stocks/snapshots?symbols=' + encodeURIComponent(symbols.map(wireSymbol).join(',')) + '&feed=' + config.feed,
      ...
      if (response.status === 429 || response.status === 503) {
        ...
        restErrorCode = 'REST_' + response.status; return;
      }
      if (response.status === 401 || response.status === 403) {
        ...
        restUntil = Infinity; restErrorCode = 'REST_AUTH'; return;
      }
      ...
      restFailures = 0; restErrorCode = null;
      for (const symbol of symbols) {
        if (!active.has(symbol)) continue;
        const record = recordFor(symbol), snapshot = data[wireSymbol(symbol)];
      ...
      if (running && epoch === generation) {
        ...
        restUntil = now() + Math.min(300000, 10000 * 2 ** (restFailures - 1)); restErrorCode = 'REST_FAILED';
      }
    } finally {
  ...
  function step() {
    if (!running) return;
    for (const [symbol, at] of active) if (now() - at >= config.activeTtlMs) { active.delete(symbol); records.delete(symbol); acknowledged.delete(symbol); }
    ...
    // Native WebSocket has no ping API. Alpaca documents an acknowledgement for
    // every subscribe message; repeating one existing subscription changes no
    // coverage and probes quiet connections without relying on trade activity.
    // https://docs.alpaca.markets/us/docs/streaming-market-data#subscription
    if(authenticated && probeAt!==null && now()-probeAt>=10000)fail('STREAM_SILENT');
    if(authenticated && probeAt===null && framesAt!==null && now()-framesAt>=30000 && now()-lastSubscriptionAt>=30000) {
      const symbol=acknowledged.values().next().value;
      if(symbol && send({action:'subscribe',trades:[wireSymbol(symbol)],quotes:[wireSymbol(symbol)]})){probeAt=now();lastSubscriptionAt=now();}
    }
  ...
  function read(symbol) {
    ...
    return {orderBook:record.book,trade:t ? {symbol:t.symbol, price:t.price, quoteAt:t.quoteAt, receivedAt:t.receivedAt,
      ...
      snapshot:record.snapshot, snapshotCheckedAt:record.snapshotCheckedAt, connectionCheckedAt:framesAt,
      connectionHealthy:!!(authenticated&&socket?.readyState===1&&framesAt!==null&&now()-framesAt<90000),delayMinutes:0,
      source:'alpaca-' + config.feed, coverage:config.feed === 'iex' ? 'single-exchange' : 'us-sip',
      state:['streaming','subscribing','subscription-limited'].includes(status)?(acknowledged.has(symbol)?'streaming':subscriptionBlocked?'subscription-limited':'subscribing'):status,
      errorCode:errorCode||restErrorCode,connectionErrorCode:errorCode,snapshotErrorCode:restErrorCode, invalidated:record.invalidThrough != null && !t};
  }
  function start() {
    if (running || !config.enabled) return;
    running = true; fatal = false; subscriptionBlocked = false; status = 'idle'; errorCode = null;restErrorCode=null;
  ...
  function retain(symbols){
    const keep=new Set(symbols);let removed=false;
    for(const s of active.keys())if(!keep.has(s)){active.delete(s);records.delete(s);acknowledged.delete(s);removed=true;}
    if(subscriptionBlocked&&removed){subscriptionBlocked=false;if(errorCode==='SYMBOL_LIMIT')errorCode=null;status=acknowledged.size===active.size?'streaming':'subscribing';lastSubscriptionAt=-Infinity;}
    if(!active.size){disconnect();status=fatal?'blocked':'idle';}else reconcile();
  }
  return Object.freeze({start, stop, step, touch, read, retain,subscribe(fn){listeners.add(fn);return ()=>listeners.delete(fn);}, supports:supported,
    diagnostics:() => ({enabled:config.enabled, feed:config.feed, status, errorCode:errorCode||restErrorCode,
      connectionErrorCode:errorCode,snapshotErrorCode:restErrorCode,active:active.size,
      ...
      connectionCheckedAt:framesAt,connectionHealthy:!!(authenticated&&socket?.readyState===1&&framesAt!==null&&now()-framesAt<90000),probeAt,
