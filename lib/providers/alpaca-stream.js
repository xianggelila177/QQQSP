...
export function createAlpacaProvider({env = {}, now = Date.now, httpsGet,
  ...
  let authenticated = false, connectingAt = 0, connectedAt = 0;
  ...
  function reconcile() {
    ...
    if (remove.length && send({action:'unsubscribe', trades:remove})) remove.forEach(symbol => requested.delete(symbol));
    if (add.length && send({action:'subscribe', trades:add})) add.forEach(symbol => requested.add(symbol));
  }
  function recordFor(symbol) {
    let record = records.get(symbol);
    if (!record) { record = {trade:null, invalidThrough:null, snapshot:null, snapshotCheckedAt:null, nextSnapshotAt:0}; records.set(symbol, record); }
    return record;
  }
  ...
  function read(symbol) {
    ...
    return {trade:t ? {symbol:t.symbol, price:t.price, quoteAt:t.quoteAt, receivedAt:t.receivedAt,
      sourceTimestamp:t.sourceTimestamp, id:t.id, exchange:t.exchange, conditions:t.conditions} : null,
      snapshot:record.snapshot, snapshotCheckedAt:record.snapshotCheckedAt,
