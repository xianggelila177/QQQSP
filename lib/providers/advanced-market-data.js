...
const barValid=b=>b&&[b.o,b.h,b.l,b.c].every(v=>finite(v)&&v>0)&&b.h>=Math.max(b.o,b.c)&&b.l<=Math.min(b.o,b.c)&&b.l<=b.h&&(b.v==null||finite(b.v)&&b.v>=0);
...
export function createAdvancedMarketData({httpsGet,fetchChart,apiKey='',apiSecret='',feed='iex',now=Date.now}={}){
 ...
 async function intraday(q,signal){
  ...
  if(configured&&marketKeyFor(q.symbol)==='us'&&['EQUITY','ETF','MUTUALFUND'].includes(instrumentTypeFor(q.symbol))){
   ...
   source='yahoo';currency=data.meta.currency||null;const v=data.indicators.quote[0];
  ...
  const byTime=new Map();for(const p of points){
   if(!finite(p.t)||p.t*1000<a||p.t*1000>b||!barValid(p)){rejected++;continue;}
 ...
 async function actions(q,signal){
  ...
  if(configured&&marketKeyFor(q.symbol)==='us'){
   ...
   if(!fetchChart)throw contextError('ACTIONS_SOURCE_NOT_CONFIGURED',503);
   const data=await fetchChart(q.symbol,'?interval=1d&period1='+Math.floor(localInstant(q.symbol,start)/1000)+'&period2='+Math.floor(localInstant(q.symbol,addDays(end,1))/1000)+'&events=div%2Csplits',{signal});
