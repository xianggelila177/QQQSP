...
export function createMacroCalendar({httpsGet,key='',now=Date.now}={}){
 ...
 function snapshot(){return {enabled:!!key,status:!key?'disabled':error?'error':inflight?'loading':updatedAt?'ready':'waiting',items:data,updatedAt,nextAt,error,
  note:key?'来源为Trading Economics；预期快照仅在本进程、面板展开时捕获，重启后不伪造发布前记录。':'结构化一致预期未配置；仍可使用官方订阅和报道证据，不将模型预测填为市场共识。'};}
 ...
 return {getCalendar:refresh,requestCalendar(){if(key&&!closed&&!inflight&&now()>=nextAt)void refresh();return snapshot();},snapshot,close(){closed=true;},reopen(){closed=false;},clear(){data=[];frozen.clear();updatedAt=null;error=null;nextAt=0;}};
}
