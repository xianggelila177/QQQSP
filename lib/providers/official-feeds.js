...
function parseRss(body, feed, nowMs) {
  ...
  for (const item of itemNodes) {
    ...
    seen.add(link); out.push({ title, source: feed.name, pubDate, link, topic: '官方公告', official: true }); if (out.length >= MAX_ITEMS_PER_SOURCE) break;
  }
  return out;
}
...
export function createOfficialFeeds({ httpsGet, now = () => Date.now(), log = () => {}, ttlMs = 1800000, failureCooldownMs = 300000 } = {}) {
  const ttl = Math.max(0, Number(ttlMs) || 0); const cooldown = Math.max(0, Number(failureCooldownMs) || 0);
  const state = new Map(FEEDS.map(feed => [feed.id, { items: [], successAt: null, lastAttemptAt: null, lastFailureAt: null, error: null, inflight: null, ttlMs: ttl }]));
  ...
  function loadOne(feed) {
    ...
    if (entry.successAt != null && age >= 0 && age < ttl) return Promise.resolve(entry.items);
