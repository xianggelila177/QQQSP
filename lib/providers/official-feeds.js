...
import {NEWS_MAX_AGE_MS} from '../news-policy.js';

const MAX_BODY_BYTES = 2 * 1024 * 1024;
const MAX_ITEMS_PER_SOURCE = 8;
const MAX_ITEMS = 18;
const MAX_AGE_MS = NEWS_MAX_AGE_MS;
...
function parseRss(body, feed, nowMs) {
  ...
  for (const item of itemNodes) {
    ...
    seen.add(link); out.push({ title, source: feed.name, pubDate, link, sourceText, topic: '官方公告', official: true });
  }
  return out.sort((a,b)=>Date.parse(b.pubDate)-Date.parse(a.pubDate)).slice(0,MAX_ITEMS_PER_SOURCE);
}
