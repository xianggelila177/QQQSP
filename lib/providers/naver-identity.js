...
export function naverSearchIdentity(data,symbol){
 ...
 const root=`/worldstock/${item.isEtf?'etf':'stock'}/${code}`;
 if(item.url!==root&&item.url!==root+'/total')return null;
 return {code,exchange,instrumentType:item.isEtf?'ETF':'EQUITY',provenance:'naver-search'};
}
...
export function createNaverIdentityResolver({request,now=Date.now}={}){
 ...
 return {peek,resolve,
  invalidate(symbol,code){const entry=cache.get(symbol);if(entry?.value?.code===code)cache.delete(symbol);},
  close(){closed=true;generation++;cache.clear();for(const job of jobs.values())job.controller.abort(stopped());},reopen(){closed=false;}};
}
