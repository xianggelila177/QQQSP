...
export function parseEastmoneyFuture(payload,symbol,now=Date.now()){
 ...
 const price=number(d.f43);if(!(price>0))throw fail('FUTURE_EMPTY','期货来源没有有效价格');
 const reference=number(d.f60),change=number(d.f169)??(reference>0?price-reference:null),at=tradeTime(d.f86,now);
 return publishQuote({...base(symbol,now),src:'eastmoney-futures',price,prevClose:reference,
  change,changePct:number(d.f170)??(reference>0&&change!=null?change/reference*100:null),changeBasis:'source-reference',
...
export function parseYahooFuture(data,symbol,now=Date.now()){
 ...
 const price=number(m.regularMarketPrice);if(!(price>0))throw fail('FUTURE_EMPTY','期货来源没有有效价格');
 ...
 const quote=data.indicators?.quote?.[0],bars=(data.timestamp||[]).map((t,i)=>({t,c:number(quote?.close?.[i]),v:number(quote?.volume?.[i])})).filter(b=>b.t>0&&b.t*1000<=now&&b.c>0);
 return publishQuote({...base(symbol,now),currency:m.currency||p?.currency||'USD',exchangeName:m.fullExchangeName||m.exchangeName||p?.exchange||'交易所依来源',
  src:'yahoo-futures',price,prevClose:previous,change:previous>0?price-previous:null,changePct:previous>0?(price-previous)/previous*100:null,changeBasis:'source-reference',
...
export function createFuturesProvider({httpsGet,fetchChart,now=Date.now,pollMs=5000}={}){
 async function json(url){
  const r=await httpsGet(url,{Referer:'https://quote.eastmoney.com/'},{family:4});
 ...
 const lists=createCachedResource({now,ttlMs:30000,retryMs:60000,maxEntries:3,loader:async market=>{
  const q=new URLSearchParams({orderBy:'dm',sort:'asc',pageSize:'1000',pageIndex:'0',token:PUBLIC_DIRECTORY_ID,field:'dm,sc,name,p,zsjd,zde,zdf,f152,o,h,l,zjsj,vol,wp,np,ccl',blockName:'callback'});
  ...
  return {rows:data.list.slice(0,1000),total:number(data.total),checkedAt:now()};
 }});
 async function load(symbol){
  const p=futureInstrumentFor(symbol);
  if(p?.provider==='eastmoney'){
   try{return parseEastmoneyFuture(await json('https://push2.eastmoney.com/api/qt/stock/get?'+new URLSearchParams({secid:p.secid,fields:'f43,f44,f45,f46,f47,f57,f58,f60,f86,f169,f170',fltt:'2',invt:'2'})),symbol,now());}
   catch(error){
    // Never evade a source rate limit through another endpoint/hostname.
    if(error.status===429||error.status===403||error.code==='SOURCE_COOLDOWN')throw error;
    const list=await lists.get(listMarket(p)),row=list.rows.find(x=>String(x.dm).toUpperCase()===p.sourceSymbol);
    if(!row||!(number(row.p)>0))throw error;
    const previous=number(row.zjsj),price=number(row.p),change=number(row.zde)??(previous>0?price-previous:null);
    return publishQuote({...base(symbol,list.checkedAt),src:'eastmoney-futures-list',price,prevClose:previous,open:number(row.o),dayHigh:number(row.h),dayLow:number(row.l),volume:number(row.vol),
     change,changePct:number(row.zdf)??(previous>0&&change!=null?change/previous*100:null),changeBasis:'previous-settlement',quoteTimePrecision:'unknown',
     priceBasis:'期货目录报价；来源没有成交时间，不按检查时间生成分时',pollAfterMs:30000,checkIntervalMs:30000});
   }
  }
 ...
 async function getQuote(symbol){
  const maxAgeMs=Math.max(pollMs,cache.cache.get(symbol)?.data?.checkIntervalMs||0);
  ...
  return {...data,pollAfterMs:interval,checkIntervalMs:interval,
 ...
 return {supports:isFutureSymbol,getQuote,search,close(){cache.close();lists.close();},reopen(){cache.reopen();lists.reopen();},diagnostics:()=>({quotes:cache.cache.size,failures:cache.failures.size,directoryPages:lists.cache.size,pollMs})};
}
