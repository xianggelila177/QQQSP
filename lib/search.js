...
export function createSearchService({ now = () => Date.now(), httpsGet, yGated, failureCooldown = 30000 } = {}) {
  ...
  async function loadSearch(key,signal) {
    ...
    const out = rows.filter(x => x && x.symbol && String(x.quoteType || '').toUpperCase() !== 'CRYPTOCURRENCY').map(x => ({
      symbol: canonicalSearchQuery(x.symbol).toUpperCase(),
      name: x.shortname || x.longname || x.symbol,
      exch: x.exchDisp || x.exchange || x.exch || '',
      type: instrumentTypeFor(x.symbol,x.quoteType || x.typeDisp || ''),
      market: classifyMarket(x.symbol, x.exchange || x.exch, x.exchDisp, x.quoteType),
    }));
  ...
  function clearSearchCache() { searchCache.clear(); failures.clear();suggest.clear();shared.close(); }   // 测试隔离(resetState 由组合根统一调用)
