...
export function createMultiSourceFundamentals({sources,now=Date.now,enabled=true,ttlMs=21600000,retryMs=300000,maxAgeMs=604800000,onUpdate=()=>{},maxEntries=128}={}){
  ...
  function wanted(spec,quote,selected){
    ...
    return spec.fields.filter(k=>!excluded.includes(k)).some(field=>{const value=selected?.fields?.[field],owner=byId.get(value?.providerId);return !value||value.stale||!owner||financialPriority(spec,field)<financialPriority(owner,field)||value.providerId===spec.id;});
  }
