...
import {normalizeFinancialRecord,selectFinancialRecord,deriveFinancialFields,financialPriority,usableFinancialField} from './financial-candidates.js';
...
export function createMultiSourceFundamentals({sources,now=Date.now,enabled=true,ttlMs=21600000,retryMs=300000,maxAgeMs=604800000,onUpdate=()=>{},maxEntries=128}={}){
  ...
  function entries(symbol){return [...specs.map(s=>({id:s.id,data:cache.get(key(s.id,symbol)),failure:failures.get(key(s.id,symbol))})),...[...liveCache.values()].filter(v=>v.symbol===symbol).map(data=>({id:'live:'+data.source,data}))];}
  function allSpecs(){return [...specs,...[...new Set([...liveCache.values()].map(r=>r.source))].map(id=>({...specs.find(s=>s.id===id),priority:specs.find(s=>s.id===id)?.priority??20,id:'live:'+id}))];}
  const select=symbol=>selectFinancialRecord(symbol,entries(symbol),allSpecs(),{now:now(),maxAgeMs});
  function trim(){while(quotes.size>maxEntries){const symbol=quotes.keys().next().value;quotes.delete(symbol);for(const s of specs){const k=key(s.id,symbol);warming.get(k)?.abort();warming.delete(k);cache.delete(k);failures.delete(k);}for(const [k,v] of liveCache)if(v.symbol===symbol)liveCache.delete(k);}}
  function wanted(spec,quote,selected){
    ...
    return spec.fields.filter(k=>!excluded.includes(k)).some(field=>{const value=selected?.fields?.[field],owner=allSpecs().find(s=>s.id===value?.providerId);return !value||value.stale||!owner||financialPriority(spec,field)<financialPriority(owner,field)||value.providerId===spec.id;});
  }
  function warm(spec,quote,selected){
    ...
    if(!wanted(spec,quote,selected))return;
    ...
    void queue.run(async signal=>{
      if(!wanted(spec,quotes.get(symbol)||quote,select(symbol)))return null;
  ...
  function decorate(quote){
    ...
    if(enabled&&live?.symbol===quote.symbol&&typeof live.source==='string'&&Number.isFinite(stamp)&&stamp<=now()+5000&&stamp>(liveCache.get(liveKey)?.fetchedAt||0)){
      try{liveCache.set(liveKey,normalizeFinancialRecord(quote.symbol,live,{source:live.source,now:stamp,ttlMs:spec?.ttlMs||60000,maxAgeMs,previous:liveCache.get(liveKey)}));}catch{}
    }
    let record=enabled?select(quote.symbol):null;
    if(security)for(const s of specs)warm(s,quote,record);
    ...
    for(const [key,field] of Object.entries(record?.fields||{}))if(['week52High','week52Low'].includes(key))f.fields[key]=field;
    ...
    const available=8-missing.length+applicable.length-missingFinancial.length,stale=Object.values(f.fields).some(v=>v.stale),loading=specs.some(s=>warming.has(key(s.id,quote.symbol)));
    f.coverage={total:24,applicable:8+applicable.length,available,missing:[...missing,...missingFinancial]};
  ...
  return {decorate,start(){running=true;for(const q of Object.values(queues))q.reopen();},stop,
    ...
    clear(){cache.clear();liveCache.clear();failures.clear();},
