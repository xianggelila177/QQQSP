...
export function quoteStatus(q,at=Date.now()) {
  ...
  const liveConnection=quoteConnectionHealthy(q,at);
  // Connection liveness and the last trade's age are separate checks. A quiet
  // but healthy stream does not need a fabricated sourceCheckedAt every tick.
  const overdue=fetched>at+1000||!liveConnection&&fetched>0&&at-fetched>Math.max(30000,cadence+15000);
