...
export const UPSTREAM_ROLES = Object.freeze({
  'polling.finance.naver.com':'realtime-snapshots',
  'qt.gtimg.cn':'cn-quote-primary','push2.eastmoney.com':'cn-quote-fallback',
  'query1.finance.yahoo.com':'global-quote-news','api.nasdaq.com':'daily-history',
  'money.finance.sina.com.cn':'cn-history-fallback','search-api-web.eastmoney.com':'cn-news',
  'news.google.com':'news-fallback','zhibo.sina.com.cn':'macro-flash',
  'smartbox.gtimg.cn':'search-suggest','fc.yahoo.com':'yahoo-auth',
  'www.ecb.europa.eu':'reference-fx-official-news','api.frankfurter.dev':'ecb-reference-mirror',
  'www.federalreserve.gov':'official-macro-fed','apps.bea.gov':'official-macro-bea',
});
...
export function quoteStatus(q,at=Date.now()) {
  ...
  const closed=['CLOSED','HOLIDAY'].includes(q.marketState);
  const ordinaryAge=Math.max(300000,(Number(q.feedDelayMinutes)||0)*60000+30000);
  let maxAge=closed?14*86400000:ordinaryAge;
  // During the actual exchange lunch break, the morning close is the latest
  // expected trade. Source checks still have their independent short deadline.
  if(q.marketState==='BREAK' && q.symbol) {
    const offset=timezoneOffsetFor(q.symbol,at);
    if(offset!=null) {
      const local=new Date(at+offset*1000),minute=local.getUTCHours()*60+local.getUTCMinutes();
      const pause=sessionFor(q.symbol,offset,at,q).session.brk?.find(([from,to])=>minute>=from&&minute<to);
      if(pause) maxAge=ordinaryAge+(minute-pause[0])*60000+local.getUTCSeconds()*1000+local.getUTCMilliseconds();
    }
  }
