...
export const MACRO_TOPICS=[
  ...
  {id:'oil',name:'原油',q:'Crude oil',tickers:['CL=F','USO','BZ=F','XOM','CVX'],kw:/oil|crude|opec/i},
...
export function createMacroService({yahooNews,httpsGet,googleNewsTopic,officialNews,cacheSet=boundedCacheSet,now=Date.now,log=defaultLog}={}) {
  ...
  async function source(key,loader) {
    const old=macroCache.get(key);
    if(old&&now()-old.attemptAt<(old.error?60000:300000))return old;
    let value;
    try{
  ...
  async function topic(tp){
    ...
    const parts=[rss];
    if(rss.items.length<2||rss.error)parts.push(await source(tp.id+':yahoo',async()=>{
      ...
      return raw.filter(n=>now()-n.t<=MACRO_MAX_AGE&&((tp.tickers&&n.tickers?.some(x=>tp.tickers.includes(x)))||tp.kw.test(n.title)))
        .map(n=>({...n,topic:tp.name,sent:sentiOf(n.title)}));
    }));
    return parts;
  }
  function adaptOfficial(item){
    ...
    return {title,src:String(item?.source || item?.src || '官方公告'),link,t,pubDate,topic:'官方公告',official:true,sent:sentiOf(title)};
  }
  ...
  async function flash(){return source('sina:flash',async()=>{
    const r=requireNewsResponse(await httpsGet('https://zhibo.sina.com.cn/api/zhibo/feed?page=1&page_size=6&zhibo_id=152&tag_id=0'),'Sina macro');
    const list=JSON.parse(r.body)?.result?.data?.feed?.list;
    if(!Array.isArray(list))throw new Error('Sina macro invalid response');
    return list.map(x=>{const title=String(x.rich_text || '').replace(/<[^>]+>/g,'').slice(0,90);return {t:new Date(String(x.create_time).replace(' ','T')+'+08:00').getTime(),src:'新浪7x24',title,link:/^https?:/.test(x.docurl || '')?x.docurl:'',topic:'快讯',sent:sentiOf(title)};}).filter(n=>n.t>0&&n.title);
  });}
  function refresh(){
    ...
    macroInflight=(async()=>{
      ...
      const seen=new Set();const unique=items.filter(n=>{const key=n.link || n.title;if(seen.has(key))return false;seen.add(key);return true;});
      ...
      const data={items:bounded,topics:[...MACRO_TOPICS.map(t=>t.name),...(officialNews?['官方公告']:[])],updated:updatedAt,updatedAt,stale:failures.length>0,refreshing:false,
