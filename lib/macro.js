...
import {filterRecentNews,combineNewsQuality,NEWS_MAX_AGE_MS} from './news-policy.js';
export const MACRO_MAX_AGE=NEWS_MAX_AGE_MS;
export const OFFICIAL_NEWS_MAX_AGE=NEWS_MAX_AGE_MS;
...
export function createMacroService({yahooNews,httpsGet,googleNewsTopic,officialNews,allowYahooFallback=true,cacheSet=boundedCacheSet,now=Date.now,log=defaultLog}={}) {
  ...
  async function source(key,loader,ttlMs=300000) {
    ...
    try{
      ...
      const checked=filterRecentNews(envelope.items,now());
      value={items:checked.items,quality:combineNewsQuality(checked.quality,envelope.quality),updatedAt:envelopeUpdatedAt,attemptAt:now(),stale:!!envelope.stale,
  ...
  async function topic(tp){
    const rss=await source(tp.id+':rss',()=>googleNewsTopic(tp,{includeQuality:true}));
    const parts=[rss];
    if(allowYahooFallback&&(rss.items.length<2||rss.error))parts.push(await source(tp.id+':yahoo',async()=>{
      ...
      return raw.filter(n=>n.t<=now()&&now()-n.t<=MACRO_MAX_AGE&&((tp.tickers&&n.tickers?.some(x=>tp.tickers.includes(x)))||tp.kw.test(n.title)))
  ...
  function refresh(){
    ...
    macroInflight=(async()=>{
      ...
      const current=now();
      const checked=filterRecentNews(parts.flatMap(p=>p.items),current);
      ...
      const data={items:bounded,quality:combineNewsQuality({...checked.quality,accepted_items:bounded.length},...parts.map(p=>p.quality)),topics:[...MACRO_TOPICS.map(t=>t.name),...(officialNews?['官方公告']:[])],updated:updatedAt,updatedAt,stale:failures.length>0,refreshing:false,analysisVersion:1,analysisBasis:'来源标题/订阅摘要的本地条件规则；不是交易所评级或收益预测',
  ...
  function view(data){const checked=filterRecentNews(data.items,now());return {...data,items:checked.items.map(item=>({...item,assessment:assessMacro(item,now())})),quality:combineNewsQuality(checked.quality,data.quality)};}
  async function getMacro({waitForRefresh=false}={}) {
    if(macroSnap&&now()-macroSnap.ts<60000)return view(macroSnap.data);
    if(!macroSnap||waitForRefresh)return refresh();
    if(lastAttemptAt==null||now()-lastAttemptAt>=60000)void refresh();
    return {...view(macroSnap.data),refreshing:!!macroInflight};
  }
