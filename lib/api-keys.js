...
export function createApiKeys({apiKey='',file='',now=Date.now}={}){
 ...
 function mutate(input){
  const work=serial.then(async()=>{
   ...
   let next=structuredClone(all());let secret=null,result;
   ...
   if(action==='create'||action==='rotate'){
    ...
    if(action==='rotate'&&(!old||old.status!=='active'||old.expiresAt!==null&&old.expiresAt<=now()))throw contextError('KEY_NOT_ACTIVE',409);
    ...
    // Keep live rotation siblings and their family IDs. Only reclaim expired
    // or revoked managed history when space is needed. Environment tombstones
    // must survive compaction or all() could resurrect a revoked env key.
    while(next.length>=256){
     const index=next.findIndex(k=>k!==old&&k.id!=='legacy'&&!k.id.startsWith('retired-env-')&&k.digest!==legacy?.digest&&
       (k.status==='revoked'||k.expiresAt!==null&&k.expiresAt<=now()));
     if(index<0)throw contextError('KEY_CAPACITY_EXCEEDED',409);
     next.splice(index,1);
    }
