...
const DEFAULT_MAX_ENTRIES = MAX_WATCHLIST_SYMBOLS;
...
const NUMERIC_KEYS = new Set([
  ...
  'feedDelayMinutes', 'pollAfterMs', 'checkIntervalMs', 'regularQuoteAt', 'previousCloseAsOf',
  'providerPrevClose',
]);
const STRING_KEYS = new Set([
  ...
  'feedDelaySource','feedDelaySourceUrl', 'quoteTradeDate', 'previousCloseTradeDate',
  'previousCloseStatus', 'previousCloseMissingReason', 'previousCloseSource', 'changeBasis',
]);
const BOOLEAN_KEYS = new Set(['ohlcConsistent', 'fxStale']);
const EXT_KEYS = new Set(['pre', 'post', 'reg']);
const EXT_NUMERIC_KEYS = new Set(['t', 'price', 'change', 'changePct', 'open', 'high', 'low', 'close', 'volume', 'vwap', 'n', 'referencePrice']);
...
function normalizeExt(value) {
  ...
  for (const key of EXT_KEYS) {
    ...
    if (typeof source.referenceTradeDate === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(source.referenceTradeDate))
      item.referenceTradeDate = source.referenceTradeDate;
    if (Object.keys(item).length) out[key] = item;
  }
  return Object.keys(out).length ? out : null;
}
...
export function createRecoveryStore({
  ...
  function remember(quote) {
    ...
    let normalized = normalizeEntry(quote, acceptedAt, ageLimit, { input: true });
    if (!normalized) return false;
    normalized.recoverySavedAt = acceptedAt;
    const previous = entries.get(normalized.symbol);
    if (previous && entryTime(normalized) < entryTime(previous)) {
      const merged=mergeQuoteHistory(previous,normalized);
      if(merged===previous)return false;
      normalized={...merged,recoverySavedAt:acceptedAt};
    }
