...
function boundMap(source, maxEntries, maxBytes) {
  ...
  while (map.size && serialize(map).bytes > maxBytes) map.delete(oldestKey(map));
  return map;
}
...
export function createRecoveryStore({
  ...
  let error = null;
  let invalidItems = 0;
  let bytes = serialize(entries).bytes;

  function setEntries(next) {
    entries = next;
    bytes = serialize(entries).bytes;
  }
  ...
  return { enabled, load, remember, get, flush, diagnostics };
}
