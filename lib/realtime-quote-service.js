...
export function mergeAlpacaQuote(base, data, now=Date.now()) {
  ...
  const sourcePreviousClose=currentDaily && validBar(prior) && priorAt && venueDate(priorAt)<day && trade.quoteAt-priorAt<10*86400000 ? prior.c : null;
  const previousClose=sourcePreviousClose??supplemental?.prevClose??null;
  ...
  const out={...inherited,symbol,displayName:base?.displayName||symbol,
    ...
    previousCloseAsOf:sourcePreviousClose!==null?priorAt:supplemental?.asOf??null,
