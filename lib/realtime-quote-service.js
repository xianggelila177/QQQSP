...
export function createRealtimeQuoteService({provider, fallback, now=Date.now, maxEntries=200, maxInflight=4,onUpdate=()=>{}}={}) {
  ...
  async function refreshFallback(symbols) {
    if(closed)return;
    await Promise.all(symbols.map(async symbol=>{
      try {
        const quote=await fallback(symbol);
        if(!closed&&quote&&!quote.error&&!quote.pending)remember(symbol,{quote,checkedAt:now(),nextAt:now()+refreshMs});
      }catch{/* A failed source check must not erase the last quote. */}
    }));
  }
