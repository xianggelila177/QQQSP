...
export function mergeAlpacaQuote(base, data, now=Date.now()) {
  ...
  if (!trade || !finitePositive(trade.price)) return base;
  ...
  const streaming=data.state==='streaming';
  const out={...(base && !base.error ? base : {}),symbol,displayName:base?.displayName||symbol,
    ...
    src:data.source,priceBasis:'reported-trade',quoteKind:'reported-trade',feedCoverage:data.coverage,feedDelayMinutes:data.delayMinutes===undefined?0:data.delayMinutes,
    ...
    stale:!streaming && now-checkedAt>120000};
  for(const key of ['error','code','pending','recovery','staleInfo','retryAt','retryAfterMs']) delete out[key];
  if(out.stale)out.staleInfo={reason:'realtime-disconnected'};
...
export function createRealtimeQuoteService({provider, fallback, now=Date.now, ttl=60000, maxEntries=200, maxInflight=4}={}) {
  ...
  function warm(symbol) {
    ...
    const work=Promise.resolve().then(()=>fallback(symbol)).then(quote=>{
      if(!closed && epoch===generation)remember(symbol,{quote:quote?.error||quote?.pending ? old?.quote : quote,nextAt:now()+(quote?.error||quote?.pending?1000:ttl)});
      return quote;
    },()=>{
      if(!closed && epoch===generation)remember(symbol,{quote:old?.quote,nextAt:now()+ttl});
  ...
  async function getCachedQuote(symbol) {
    ...
    if(data?.state!=='streaming'&&cache.has(symbol))cache.get(symbol).nextAt=Math.min(cache.get(symbol).nextAt,now()+1000);
