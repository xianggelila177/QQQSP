...
export function createHostGate({now=Date.now,minGap=minimumGap,baseMs=30000,capMs=900000,maxQueued=32,deadlineMs=20000}={}) {
  ...
  function state(key) {
    if(!hosts.has(key))hosts.set(key,{queue:createTaskQueue({maxActive:1,maxQueued,now}),lastStart:-Infinity,until:0,failures:0,busy:false,calls:0,rateLimits:0,suppressed:0,lastStatus:null});
  ...
  function run(url,work,{signal,deadlineAt=now()+deadlineMs}={}) {
  ...
  function diagnostics() {
    return Object.fromEntries([...hosts].map(([host,s])=>[host,{calls:s.calls,rateLimits:s.rateLimits,suppressed:s.suppressed,lastStatus:s.lastStatus,retryAt:s.until>now()?s.until:null,
      state:s.until>now()?'cooldown':s.busy&&s.failures?'probing':s.busy?'requesting':'ready',failures:s.failures,minGapMs:minGap(host),...s.queue.diagnostics()}]));
  }
