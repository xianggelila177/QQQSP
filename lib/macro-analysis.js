...
function newsFragments(item){
 if(Array.isArray(item.groupFragments))return item.groupFragments.filter(f=>f&&typeof f==='object').map(f=>({...f}));
 return [{title:item.title,sourceText:item.sourceText||item.title,t:item.t,src:item.src,link:item.link,linkScope:item.linkScope,official:!!item.official,topic:item.topic}];
}
function joinNewsFragments(...groups){const seen=new Set();return groups.flat().filter(f=>{const key=JSON.stringify([f.title,f.sourceText,f.t,f.link]);if(seen.has(key))return false;seen.add(key);return true;}).slice(0,8);}
export function mergeMacroItems(items){
 ...
 for(const item of items){
  ...
  if(old){if(group){old.groupFragments=joinNewsFragments(newsFragments(old),newsFragments(item));old.groupedCount=old.groupFragments.length;old.sourceText=old.groupFragments.map(f=>f.sourceText||f.title).join('。').slice(0,1800);old.reportingPeriod={firstPublishedAt:Math.min(old.reportingPeriod?.firstPublishedAt??old.t,item.t),latestPublishedAt:Math.max(old.reportingPeriod?.latestPublishedAt??old.t,item.t)};}if(!old.reports.some(r=>r.link===link&&r.src===report.src))old.reports.push(report);if(item.official&&!old.official){Object.assign(old,item,{reports:old.reports});}if(identityLink)links.set(identityLink,old);continue;}
  const row={...item,reports:item.reports||[report],...(group?{groupFragments:newsFragments(item),groupedCount:item.groupFragments?.length||1}:{}),independentConfirmations:null};out.push(row);if(identityLink)links.set(identityLink,row);titles.set(key,row);if(group)surveys.set(group,row);
 }
 return out;
}
