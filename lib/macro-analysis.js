...
const MAYBE=/可能|预计|预料|传闻|据悉|考虑|或将|有望|将于|尚未|否认|没有|不会|\b(may|might|could|denies|denied|rumou?r|consider|preview|forecast to|expected to)\b/i;
...
export function assessMacro(item,now=Date.now()){
 ...
 const x={schemaVersion:1,ruleVersion:'macro-evidence-1',event:'background',status:'watch',importance:'background',scope:excerpt?'feed-excerpt':'title',evidenceTier:official(item)?'official':'reported',
  publishedAt,topic:OIL.test(text)?'原油':INFLATION.test(text)?'通胀':RATES.test(text)?'利率':'其他',summary:'没有足够的事件数据支持方向判断。',
 ...
 if(INFLATION.test(text)){
  ...
  if(x.releases.length){
   x.event='inflation-surprise';x.importance='focus';
 ...
 if(OIL.test(text)){
  ...
  if(['oil-demand-loss','oil-supply-loss','oil-supply-gain'].includes(x.event))x.importance='focus';
...
export function mergeMacroItems(items){
 const out=[],links=new Map(),titles=new Map();
 for(const item of items){
  ...
  const old=(identityLink&&links.get(identityLink))||titles.get(key);
  const report={src:item.src||'未知来源',link,publishedAt:item.t};
  if(old){if(!old.reports.some(r=>r.link===link&&r.src===report.src))old.reports.push(report);if(item.official&&!old.official){Object.assign(old,item,{reports:old.reports});}if(identityLink)links.set(identityLink,old);continue;}
  const row={...item,reports:[report],independentConfirmations:null};out.push(row);if(identityLink)links.set(identityLink,row);titles.set(key,row);
 }
 return out;
}
