...
export function createApplication({env={},now=()=>Date.now(),telemetry:providedTelemetry,transport:providedTransport,upstream=null,providerOverrides={}}={}) {
  ...
  function stop(){
    ...
    indexProvider.close();fundamentals.stop();engine.stop();news.stopNews();realtime?.stop();snapshots?.stop();auth.close();history.close();futures.close();macroQuotes.close();macroContext.close();macroCalendar.close();quoteCache.close();for(const service of [tx,nasdaq,sina,quote])service.close();
