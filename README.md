# QQQSP 2.16.3 · 多市场行情与历史图表

## 一、版本概览

应用版本 **2.16.3**，静态资源版本 **108**。配置自有免费 Finnhub API Key 后，成交流和网站批量轮询共同提供美股报价。新值有变化即推送，面板与本地 Agent 共享同一份数据；保留近7天新闻、全球证券搜索及原有接口。

v108 修复跨证券盘口残留、历史与图表质量状态、期货基准和价格值域，统一详情请求策略，并减少重复图表传输与计算。后台仅预热近期历史，周/月/年视图按需补齐。发布入口统一为 `scripts/package.py`，旧 `package-release.py` 仅作兼容转发；打包拒绝私有目录和非 LF Shell，回滚验证就绪并在失败时恢复。详细清单与验证见 [v108 审查修复](docs/AUDIT-FIXES-v108.md)。README 的资源版本随 `npm run build` 自动同步。
