...
test('real HTTP full/same/tail chart contract, partial news metadata and quota recovery', async ({ page, panelServer }) => {
  ...
  const response = await page.request.get(panelServer.url+'/api/market?symbols=QQQ,SPY');
  ...
  panelServer.app.services.engine.poke('QQQ');
  await expect.poll(()=>panelServer.app.services.engine.read(['QQQ'],{lease:false})[0]?.charts?.intraday?.at(-1)?.c).toBe(initial.charts.intraday.at(-1).c+3);
  const changed = (await (await page.request.get(panelServer.url+'/api/market?symbols=QQQ,SPY')).json())[0];
  await page.evaluate(() => window.__hooks.refresh(true));
  ...
  await card.locator('.newshead').click();await page.locator('.price-card[data-sym="SPY"] .newshead').click();
