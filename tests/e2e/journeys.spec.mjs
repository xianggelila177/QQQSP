...
test('macro cache, error/retry status and stable keyboard focus', async ({ page, panelServer }) => {
  ...
  await expect(page.locator('#macroStatus')).toContainText(/不可用|旧|延迟|缓存/);
...
test('simulated background timer throttling retains worker refresh and foreground catch-up', async ({ page, panelServer }) => {
  ...
  const cadence=await page.evaluate(()=>window.__hooks.currentPollingPolicy().marketMs);
  await expect.poll(() => panelServer.state.marketRequests, { timeout: cadence+1000 }).toBeGreaterThan(hiddenRequests);
});
