...
test('mobile layout, fractional DPR, keyboard OHLC and currency preservation', async ({ page, panelServer }, testInfo) => {
  ...
  await expect(card.locator('.chart-point')).toContainText(/价/);
  await card.locator('[data-tf="daily30"]').click();
  await expect(card.locator('.chart-state')).toContainText('交易所交易日');
