...
export async function createRealServer({snapshots=false,startupPartial=false,fixedNow,snapshotMarketState='CLOSED'}={}) {
  ...
  const app = createApplication({
    env: { NODE_ENV: 'test', PORT: '0', SYMBOLS: snapshots?'QQQ':'!', CACHE_MS: '1', QUOTE_MAX_AGE: '1', NEWS_TTL: '1', HTTP_SNAPSHOT_QUOTA: '30', HTTP_CLIENT_QUOTA: '10000', REALTIME_SNAPSHOTS: snapshots?'1':'0', PUBLIC_SOURCE_REDUNDANCY: '0' },
    now, upstream, providerOverrides: {
      fetchChart: async symbol => {
        const result=quote(symbol,state), bars=result.charts.daily30;
        return {meta:{symbol,dataGranularity:'1d',currency:result.currency,instrumentType:result.instrumentType},timestamp:bars.map(b=>b.t),indicators:{quote:[Object.fromEntries([['open','o'],['high','h'],['low','l'],['close','c'],['volume','v']].map(([name,key])=>[name,bars.map(b=>b[key])]))]}};
      },
  ...
  app.services.news.__impl.newsFor = async symbol => {
