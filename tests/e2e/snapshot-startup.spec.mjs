...
test('closed-market cold fallback visibly recovers missing intraday before normal fifteen-minute enrichment',async({page,panelServer})=>{
 ...
 const partial=await read();expect(partial.slowFields.intraday.maxAgeMs).toBe(1800000);expect(partial.price).toBeGreaterThan(0);expect(partial.charts.intraday).toEqual([]);expect(partial.charts.daily30.length).toBeGreaterThan(0);
