...
""")
  ...
  # Since 2.10, missing source history stays empty. Sampling is an explicit
  # separate mode; the latest quote must not be relabeled as source history.
  assert modes[1]['sampled'] is False and not modes[1]['n'],modes
  assert page.evaluate("!__app.cardCache.get('INTC').plot && !__app.cardCache.get('INTC')._sampled"),modes
  ...
  # Available source histories must actually paint. INTC intentionally has
  # no source history and must remain empty instead of fabricating a chart.
  ...
  assert all(x['pixels']>20 for x in report['painted'] if x['symbol']!='INTC'),report['painted']
  report['missingSourceHistoryRemainsEmpty']=page.evaluate("!__app.cardCache.get('INTC').plot && !__app.cardCache.get('INTC')._sampled")
  assert report['missingSourceHistoryRemainsEmpty'],report['painted']
  widths=[]
