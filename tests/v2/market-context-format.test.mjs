...
test('news and macro output whitelisted observations, safe links and source dates with no internal metadata',()=>{
 ...
 assert.deepEqual(Object.keys(news[0]),['title','source','published_at_ms','url']);assert.equal(news[0].published_at_ms,TRADE);assert.equal(news[1].url,null);assert.equal(result.sections.macro.data.factors[0].price,61.1234);assert.equal(result.sections.macro.data.factors[0].quote_at_ms,TRADE);
