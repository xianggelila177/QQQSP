...
async function temp(t){const dir=await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(),'qqqsp29-')));t.after(()=>fs.rm(dir,{recursive:true,force:true}));return dir;}
...
test('R10 background preparation stays near while wide history is requested on demand',async()=>{
 ...
 const worker=createHistoryPrewarm({history:h,now:()=>clock,tickMs:600000,expandAfterMs:5000});worker.retain(['NVDA','MRVL']);await worker.start();await worker.settled();await worker.runDue();assert.deepEqual(calls,[['NVDA','near'],['MRVL','near']]);clock+=5001;await worker.runDue();await worker.runDue();assert.equal(calls.length,2);clock+=60000;await worker.runDue();await worker.runDue();assert.deepEqual(calls.slice(2),[['NVDA','near'],['MRVL','near']]);await worker.stop();
});
