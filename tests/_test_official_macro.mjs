...
const rssItem = (title, t, extra = {}) => ({ title, t, link: 'https://normal.example/' + encodeURIComponent(title), topic: '美联储', src: 'Reuters', ...extra });
...
async function main() {
  ...
  {
    const official = makeOfficial([officialItem('official-old', 6), officialItem('official-2', 4), officialItem('official-3', 2)]);
    const svc = createMacroService({ now: () => NOW, googleNewsTopic: normalTopics, yahooNews: async () => [], httpsGet: noFlash, officialNews: official, log: { debug() {} } });
    ...
    const old = got.find(x => x.title === 'official-old'); assert.equal(old.src, 'Federal Reserve'); assert.equal(old.topic, '官方公告'); assert.equal(old.pubDate, new Date(NOW - 6 * 86400000).toUTCString()); assert.equal(old.t, Date.parse(old.pubDate));
  }

  console.log('[M3] all sources use strict seven-day publication windows');
  {
    ...
    assert.equal(r.items.some(x => x.title === 'official-29d'), false); assert.equal(r.items.some(x => x.title === 'future'), false); assert.equal(r.items.some(x => x.title === 'normal-49h'), true); assert.ok(r.items.some(x => x.title === 'normal-now'));
  }
  ...
  {
    const official = makeOfficial(Array.from({ length: 18 }, (_, i) => officialItem('official-outage-' + i, (i + 1) / 3)), { stale: true, error: 'official partial' });
    const svc = createMacroService({ now: () => NOW, googleNewsTopic: async () => { throw new Error('google down'); }, yahooNews: async () => { throw new Error('yahoo down'); }, httpsGet: async () => ({ status: 503, body: 'down' }), officialNews: official, log: { debug() {} } }); const r = await svc.getMacro({ waitForRefresh: true });
