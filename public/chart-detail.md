...
- `chart.bars`、`fiveDay.bars` 只保留交易日历定义的常规会话。`regularSessions` 保存毫秒时间边界；`tradeDate` 是图表日，可能与最新报价交易日不同。分钟 bar 使用起始时间，16:00 开始的桶不作为常规 bar。
- `chart.previousCloseReference` 使用图表日的上一交易日常规收盘；五日使用第一日的前一交易日收盘。缺失时数值为空，不以首个价格点代替昨收百分比。
- `fiveDay.days` 恒列出五个交易日槽位，每日标明 `ready`、`partial` 或 `missing`。`coveredDays` 只计确实拿到价格数据的交易日。优先使用已授权 Alpaca 同源 1 分钟 OHLCV；否则尝试 Yahoo 5 分钟 OHLCV。IEX 是单交易所覆盖，SIP 是汇总覆盖；不同来源不拼接成同一五日曲线。
- `volumeQuality` 区分已核验区间成交量、部分覆盖和缺失。Nasdaq 价格点没有区间量时 `v=null`；`0` 是有效零成交量。Yahoo、Nasdaq 历史及 Alpaca IEX/SIP 的美股股票与 ETF 股数经过来源契约核验；其他单位保持未知。
